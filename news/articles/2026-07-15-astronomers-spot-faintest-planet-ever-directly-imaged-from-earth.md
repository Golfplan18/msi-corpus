---
headline: Astronomers spot faintest planet ever directly imaged from Earth
secondary_headline: Webb telescope confirms dim world after two observations
publish_date: '2026-07-15'
lede: Astronomers have detected the faintest planet ever directly imaged from Earth
  — a cold gas giant orbiting the young star Beta Pictoris after more than a decade
  of searching — two independent research teams reported Wednesday. The discovery
  was made days apart in late 2025 by a Scottish and German-led team using the European
  Southern Observatory's Very Large Telescope in Chile and a separate California-led
  team using NASA's Webb Space Telescope.
nut_graf: The planet's extreme faintness, overshadowed by its parent star and two
  brighter companion worlds, makes it the dimmest exoplanet captured directly from
  ground- or space-based observatories. Both teams published their findings in the
  Astrophysical Journal Letters.
primary_entities:
- Beta Pictoris
- Very Large Telescope
- Webb Space Telescope
- European Southern Observatory
- NASA
primary_themes:
- exoplanet discovery
- direct imaging
- astronomy
- space science
topic_tags:
- achievement
- science and technology
- natural science
- space programme
storyline_nexus: []
geographic_location: United States
floor_values_engaged:
- value: human_life_and_dignity
  intensity: 0.9
- value: equality_fairness
  intensity: 0.3
- value: accountability_of_power
  intensity: 0.6
framework_version: 1.4.1
generation_timestamp: '2026-07-15T23:24:05-07:00'
source_cluster_id: cluster_ap_2026-07-15_planet-star-nasa-webb-eso-large-telescop
gdelt_event_ids: []
consensus_floor_version: v0.3.0
publication_mindspec_version: v0.3.0
license: https://creativecommons.org/publicdomain/zero/1.0/
ai_disclosure: This article was generated algorithmically by Main Street Independent
  from the public sources listed in its Sources section.
ai_generated: true
sources:
  count: 1
  outlets:
  - Associated Press
  outlet_classes:
  - wire
  highest_reliability_tier: 1
  has_originating: true
  has_primary_document: false
figures_aggregate:
  count: 0
  series_ids: []
  sources: []
cross_article_links:
- slug: 2026-07-08-new-horizons-probe-wakes-after-record-hibernation-beyond-pluto
  relation: related
  strength: 0.5263
  confidence: medium
- slug: 2026-07-08-royal-canadian-geographical-society-releases-first-images-of-quest-wreck
  relation: related
  strength: 0.5155
  confidence: medium
- slug: 2026-07-02-expedition-to-survey-shackleton-scott-shipwrecks-launches
  relation: related
  strength: 0.5209
  confidence: medium
- slug: 2026-05-05-astronomers-detect-an-atmosphere-around-a-mini-pluto
  relation: related
  strength: 0.6276
  confidence: medium
- slug: 2026-05-05-astr-nomos-reportan-atm-sfera-en-un-mini-mundo-helado-m-s-all-de-plut-n
  relation: related
  strength: 0.5884
  confidence: medium
- slug: 2026-05-10-webb-telescope-captures-brilliant-heart-of-spiral-galaxy-45-million-light-years-
  relation: related
  strength: 0.5719
  confidence: medium
- slug: 2026-05-20-neptune-s-moon-nereid-may-be-an-original-companion-webb-finds
  relation: related
  strength: 0.5275
  confidence: medium
- slug: 2026-05-10-nasa-s-webb-telescope-captures-messier-77-s-active-nucleus-in-infrared
  relation: related
  strength: 0.5464
  confidence: medium
- slug: 2026-04-25-updated-chile-telescope-image-shows-sombrero-galaxy-in-greater-detail
  relation: related
  strength: 0.5451
  confidence: medium
- slug: 2026-05-15-nasa-psyche-spacecraft-slingshots-past-mars-en-route-to-rare-metal-asteroid
  relation: related
  strength: 0.5007
  confidence: medium
- slug: 2026-04-24-comet-3i-atlas-likely-came-from-cold-isolated-milky-way-region
  relation: related
  strength: 0.5368
  confidence: medium
- slug: 2026-04-28-atacama-desert-skies-in-chile-face-light-pollution-pressure
  relation: related
  strength: 0.5167
  confidence: medium
- slug: 2026-01-26-james-webb-telescope-maps-dark-matter-with-unprecedented-detail
  relation: related
  strength: 0.6028
  confidence: medium
- slug: 2026-02-23-six-planets-to-parade-across-night-sky-at-month-s-end
  relation: related
  strength: 0.5857
  confidence: medium
- slug: 2026-02-26-telescope-in-chile-captures-milky-way-heart-in-new-image
  relation: related
  strength: 0.5614
  confidence: medium
- slug: 2026-04-04-artemis-ii-astronauts-share-first-images-as-they-speed-toward-the-moon
  relation: related
  strength: 0.5011
  confidence: medium
- slug: 2026-02-16-first-2026-ring-of-fire-solar-eclipse-to-be-visible-from-antarctica
  relation: related
  strength: 0.5144
  confidence: medium
- slug: 2026-01-22-seismic-networks-help-track-falling-space-debris
  relation: related
  strength: 0.5219
  confidence: medium
- slug: 2026-02-19-surprise-shark-sighting-in-antarctica-s-near-freezing-deep
  relation: related
  strength: 0.5031
  confidence: medium
draft: false
related_stories: []
image:
  url: /articles/2026-07-15-astronomers-spot-faintest-planet-ever-directly-imaged-from-earth.png
  alt: 'Illustration accompanying article: Astronomers spot faintest planet ever directly
    imaged from Earth'
  credit: Main Street Independent (algorithmic)
  source: ai_generated
  license: https://creativecommons.org/publicdomain/zero/1.0/
  disclosure: AI-generated image. Prompt summary and model identifier available in
    metadata.
  ai_model: openrouter:openai/gpt-5.4-image-2
  ai_prompt: hand-drawn, black ink linework, solid white background, fully opaque,
    linework dominant, varied pen technique | square 1:1 composition, full-bleed square
    frame that fills the entire canvas edge to edg
---
**Subtype:** fact

- A Scottish and German-led team detected a faint cold gas giant orbiting Beta Pictoris using the European Southern Observatory's Very Large Telescope in Chile, then confirmed its orbit from archival data spanning 11 years.
- A separate California-led team independently spotted the same planet with NASA's Webb Space Telescope, requiring just two observations to make the discovery.
- The planet is the dimmest world ever directly imaged from Earth, having been hidden by the glare of its parent star and two brighter companion planets.
- European Southern Observatory's Markus Bonse, who co-led the first team, said the search "was very much playing hide-and-seek for 11 years."

## Webb telescope confirms dim world after two observations

Astronomers have identified the faintest exoplanet ever directly imaged from Earth, a cold gas giant orbiting the young star Beta Pictoris, after more than a decade of searching. The planet had remained invisible in telescope data because its light was drowned out by its much brighter host star and by two previously known companion planets.

The first detection came late last year from a team led by researchers in Scotland and Germany. Using the European Southern Observatory's Very Large Telescope in Chile, they spotted the faint object and then combed through archive data going back 11 years to confirm that its motion matched an orbital path around the star. The planet was present in those older observations all along, but had been overlooked.

Days later, a team based in California independently detected the same world using NASA's Webb Space Telescope. The Webb telescope needed only two observation sessions to confirm the presence of the planet.

Both teams have reported their findings in the Astrophysical Journal Letters.

Beta Pictoris hosts at least three planets. The newly found world is the dimmest of the three and is classified as a cold gas giant.

Markus Bonse of the European Southern Observatory, who co-led the initial team, described the long hunt: "It was very much playing hide-and-seek for 11 years."

## Sources

### src_001 — Associated Press, wire, Tier 1, originating
**Author:** Marcia Dunn
**Publication date:** 2026-07-15
**Title:** Faint new planet is found after cosmic hide-and-seek
**URL:** https://apnews.com/article/planet-star-nasa-webb-eso-large-telescope-46f390e8d4e3cafeabf5b463f80811d4
