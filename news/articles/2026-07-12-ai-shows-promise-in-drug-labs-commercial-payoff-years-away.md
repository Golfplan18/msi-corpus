---
headline: AI shows promise in drug labs; commercial payoff years away
secondary_headline: Only one in 10 drug candidates reaches market
publish_date: '2026-07-12'
lede: Artificial intelligence is already making pharmaceutical research more efficient
  by predicting protein structures and screening molecules, but Wall Street has yet
  to see evidence that the technology can improve the financial returns of drug development,
  according to scientists, executives and investors.
nut_graf: The gap between AI's scientific potential and its commercial impact reflects
  the fundamental challenge of turning laboratory discoveries into medicines that
  work in people, a process that currently sees only about 10% of drug candidates
  survive clinical trials.
primary_entities:
- Goldman Sachs
- Roche
- Eli Lilly
- Novartis
- Anthropic
- Aviv Regev
- Jack Scannell
- Eric Kauderer-Abrams
primary_themes:
- Artificial intelligence
- Drug discovery
- Pharmaceutical industry
- Research and development
- Investment returns
topic_tags:
- artificial intelligence
- biotechnology
- medical research
- economy, business and finance
storyline_nexus:
- technology-ai
- health-pharma
geographic_location: United States
floor_values_engaged:
- value: human_life_and_dignity
  intensity: 0.6
- value: truthfulness
  intensity: 0.6
- value: informed_citizenship
  intensity: 0.6
framework_version: 1.4.1
generation_timestamp: '2026-07-12T02:58:39-07:00'
source_cluster_id: cluster_wsj_2026-07-12_tech-ai-can-ai-make-better-drugs-not-on-
gdelt_event_ids: []
consensus_floor_version: v0.3.0
publication_mindspec_version: v0.3.0
license: https://creativecommons.org/publicdomain/zero/1.0/
ai_disclosure: This article was generated algorithmically by Main Street Independent
  from the public sources listed in its Sources section.
ai_generated: true
sources:
  count: 1
  outlets:
  - The Wall Street Journal
  outlet_classes:
  - national_daily
  highest_reliability_tier: 1
  has_originating: true
  has_primary_document: false
figures_aggregate:
  count: 0
  series_ids: []
  sources: []
cross_article_links:
- slug: 2026-06-29-ai-erodes-skill-barrier-for-cyber-attacks-security-expert-warns
  relation: related
  confidence: medium
- slug: 2026-06-18-wall-street-grapples-with-ai-hiring-dilemma-as-tools-reshape-banking-jobs
  relation: related
  confidence: medium
- slug: 2026-06-24-ai-entrepreneurs-pivot-from-chatbots-to-building-physical-ai-systems
  relation: related
  confidence: medium
- slug: 2026-06-19-either-robots-kill-us-or-russians-ai-warfare-hits-point-of-no-return
  relation: related
  confidence: medium
- slug: 2026-06-18-utah-pilot-lets-ai-renew-prescriptions-drawing-physician-safety-concerns
  relation: related
  confidence: medium
- slug: 2026-06-10-economists-mostly-agree-ai-will-boost-productivity-split-on-job-impact
  relation: related
  confidence: medium
- slug: 2026-06-19-memory-chip-shortage-driven-by-ai-demand-squeezes-consumer-tech
  relation: related
  confidence: medium
- slug: 2026-06-13-experts-warn-ai-could-double-gdp-growth-while-fueling-underemployment-and-unrest
  relation: related
  confidence: medium
- slug: 2026-06-11-ai-convenience-may-harm-health-researchers-warn
  relation: related
  confidence: medium
- slug: 2026-06-11-base-rate-analysis-casts-doubt-on-ai-investment-frenzy-as-revenue-projections-hi
  relation: related
  confidence: medium
- slug: 2026-06-09-wall-street-funnels-record-capital-to-ai-build-out-across-equity-and-debt-market
  relation: related
  confidence: medium
- slug: 2026-06-02-ai-economy-could-create-permanent-underclass-harvard-economist-warns
  relation: related
  confidence: medium
- slug: 2026-06-01-companies-link-ai-adoption-to-promotions-amid-unclear-rollout-strategies
  relation: related
  confidence: medium
- slug: 2026-05-23-musk-altman-trial-exposes-ai-s-deep-funding-needs-raising-profit-vs-purpose-ques
  relation: related
  confidence: medium
- slug: 2026-05-20-overworked-special-education-teachers-turn-to-ai-to-write-ieps
  relation: related
  confidence: medium
- slug: 2026-05-10-ai-agent-mona-runs-swedish-cafe-as-real-world-test-of-autonomy
  relation: related
  confidence: medium
- slug: 2026-05-02-workers-describe-how-they-use-ai-to-save-time-and-develop-ideas
  relation: related
  confidence: medium
- slug: 2026-04-28-ai-anxiety-upends-college-plans-as-students-scramble-for-ai-proof-majors
  relation: related
  confidence: medium
- slug: 2026-04-13-more-u-s-workers-try-ai-at-work-but-job-loss-fears-and-ethics-linger
  relation: related
  confidence: medium
- slug: 2026-04-19-trump-opposes-state-ai-regulations-but-republican-lawmakers-push-back
  relation: related
  confidence: medium
draft: false
related_stories:
- slug: 2026-06-18-wall-street-grapples-with-ai-hiring-dilemma-as-tools-reshape-banking-jobs
  headline: Wall Street grapples with AI hiring dilemma as tools reshape banking jobs
  publish_date: '2026-06-18'
  relation: related
  strength: 0.6797
- slug: 2026-06-19-either-robots-kill-us-or-russians-ai-warfare-hits-point-of-no-return
  headline: '‘Either robots kill us or Russians’: AI warfare hits point of no return'
  publish_date: '2026-06-19'
  relation: related
  strength: 0.6642
- slug: 2026-06-10-economists-mostly-agree-ai-will-boost-productivity-split-on-job-impact
  headline: Economists mostly agree AI will boost productivity, split on job impact
  publish_date: '2026-06-10'
  relation: related
  strength: 0.6974
- slug: 2026-06-02-ai-economy-could-create-permanent-underclass-harvard-economist-warns
  headline: AI economy could create permanent underclass, Harvard economist warns
  publish_date: '2026-06-02'
  relation: related
  strength: 0.6689
- slug: 2026-06-01-companies-link-ai-adoption-to-promotions-amid-unclear-rollout-strategies
  headline: Companies link AI adoption to promotions amid unclear rollout strategies
  publish_date: '2026-06-01'
  relation: related
  strength: 0.6667
- slug: 2026-05-23-musk-altman-trial-exposes-ai-s-deep-funding-needs-raising-profit-vs-purpose-ques
  headline: Musk-Altman trial exposes AI's deep funding needs, raising profit vs.
    purpose question
  publish_date: '2026-05-23'
  relation: related
  strength: 0.677
- slug: 2026-04-13-more-u-s-workers-try-ai-at-work-but-job-loss-fears-and-ethics-linger
  headline: More U.S. workers try AI at work, but job-loss fears and ethics linger
  publish_date: '2026-04-13'
  relation: related
  strength: 0.6679
---
## AI shows promise in drug labs; commercial payoff years away

**Subtype:** fact

- AI is making drug research labs more productive by predicting protein structures and screening molecules, but the metric that matters most — how many successful drugs emerge from each research dollar — has yet to improve, according to scientists and investors.
- Only about one in 10 drug candidates that enter human trials ultimately reaches the market, with many failures coming after years of research and billions of dollars of investment.
- Goldman Sachs estimates the present value of AI's benefits to drug development could reach $400 billion over the next decade by shortening timelines and improving success rates.
- Biotech investor Rod Wong of RTW Investments says competition from China, where companies move faster from research to clinical evidence, could force the U.S. to reconsider its slow and expensive trial system.

## Only one in 10 drug candidates reaches market

Artificial intelligence has already accelerated parts of the drug discovery process that humans know how to do — predicting protein folding, identifying potential drug targets and screening millions of molecules virtually — but the technology has yet to demonstrate it can change the core economics of drug development, according to industry executives, researchers and investors.

At Roche's Genentech, computational biologist Aviv Regev said she has built a "lab in the loop" in which AI models predict promising targets and molecules, researchers test those predictions experimentally, and the resulting data improve the next round of models. "AI is not smarter," Regev said in an interview. "But what helps our scientists is that it encodes information very, very broadly."

The approach has expanded the range of research programs scientists can realistically pursue, Regev said. But expanding discovery does not directly address the industry's most persistent problem: turning ideas into medicines that work in people. Only about one in 10 drug candidates that enter human trials reach the market, and failures often occur after years of research and billions of dollars have been spent.

Jack Scannell, who co-wrote the 2012 paper that coined the term Eroom's Law — Moore's Law spelled backward — described the data challenge facing AI in drug development. Training an AI on today's biological data, he said, is "like trying to train your Waymo for San Francisco by getting a frog to ride a bike around Albuquerque." Whereas autonomous vehicles have millions of miles of real-world driving data, medicine lacks a clean map of the human body, he added.

The distinction between AI's laboratory productivity and its financial impact creates a dilemma for investors, the Wall Street Journal reported. Pharma CEOs including Eli Lilly's David Ricks and Novartis's Vas Narasimhan are committing billions to computing power and new research platforms. Goldman Sachs estimates that the present value of AI's benefits to drug development could reach $400 billion over the next decade by shortening timelines, lowering costs and improving the odds that medicines succeed, according to the report.

Eric Kauderer-Abrams, who leads life sciences at AI company Anthropic, said the industry is in the "second inning" of AI's potential. He said AI could bend the curve by attacking multiple bottlenecks to boost a drug's clinical probability of success, but acknowledged that proving its potential will take several more years.

Biotech investor Rod Wong, managing partner at RTW Investments, said competition from China could become a catalyst for change. China's advantage, he said, is in the speed at which companies can move from research ideas to clinical evidence. That pressure, he said, could force the U.S. to rethink a clinical trial system that has become increasingly slow and expensive.

If those pieces come together — richer human data, better ways to monitor patients, and faster clinical trials — Eroom's Law might finally begin to bend, the report concluded. But the winners may not be the companies that build AI models, but rather the largest established pharmaceutical companies that combine powerful tools with deep biological data and global development programs, according to Citi healthcare strategist Traver Davis. Biology runs on its own clock, not the semiconductor cycle. The revolution might yet be real, but we won't know for certain for several more years.

## Sources

### src_001 — The Wall Street Journal, national_daily, Tier 1, originating
**Author:** David Wainer
**Publication date:** 2026-07-12
**Title:** Can AI Make Better Drugs? Not on Wall Street’s Timeline
**URL:** https://www.wsj.com/tech/ai/can-ai-make-better-drugs-not-on-wall-streets-timeline-98a50d9d
