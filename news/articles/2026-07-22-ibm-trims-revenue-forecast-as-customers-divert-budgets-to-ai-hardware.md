---
headline: IBM trims revenue forecast as customers divert budgets to AI hardware
secondary_headline: Mainframe sales drop 42% after enterprise clients shift spending
  to servers and memory
publish_date: '2026-07-22'
lede: IBM said Wednesday that its full-year revenue growth will land between 4% and
  5%, a downgrade from the more than 5% it had previously forecast, as customers redirected
  budgets toward artificial intelligence hardware and paused data center deals. The
  company confirmed the disappointing results it had previewed July 14 in a rare early
  disclosure that erased about $67 billion in market value.
nut_graf: The warning signals that the AI infrastructure boom is now displacing spending
  on traditional enterprise hardware, squeezing a company whose mainframe business
  has long been a reliable revenue driver. Chief Executive Arvind Krishna said the
  company is in the "early innings of a structural shift for business" but faces an
  extended period in which customers prioritize servers and storage for AI over upgrades
  to IBM's legacy systems.
primary_entities:
- IBM
- Arvind Krishna
- James Kavanaugh
- Wall Street Journal
primary_themes:
- artificial intelligence
- enterprise technology
- earnings
- capital spending
topic_tags:
- artificial intelligence
- computing and information technology
- economy, business and finance
- business information
storyline_nexus:
- artificial-intelligence-economy
geographic_location: United States
floor_values_engaged:
- value: equality_fairness
  intensity: 0.9
- value: truthfulness
  intensity: 0.6
- value: accountability_of_power
  intensity: 0.3
framework_version: 1.4.1
generation_timestamp: '2026-07-22T14:22:49-07:00'
source_cluster_id: cluster_wsj_2026-07-22_business-earnings-ibm-q2-earnings-report
gdelt_event_ids: []
consensus_floor_version: v0.3.0
publication_mindspec_version: v0.3.0
license: https://creativecommons.org/publicdomain/zero/1.0/
ai_disclosure: This article was generated algorithmically by Main Street Independent
  from the public sources listed in its Sources section.
ai_generated: true
sources:
  count: 1
  outlets:
  - The Wall Street Journal
  outlet_classes:
  - national_daily
  highest_reliability_tier: 1
  has_originating: true
  has_primary_document: false
figures_aggregate:
  count: 0
  series_ids: []
  sources: []
cross_article_links:
- slug: 2026-07-17-ibm-shares-plunge-25-after-board-debated-early-profit-warning
  relation: related
  strength: 0.7528
  confidence: medium
- slug: 2026-07-13-oxford-nanopore-shares-slide-11-after-revenue-below-management-expectations
  relation: related
  strength: 0.6264
  confidence: medium
- slug: 2026-06-23-tech-shares-tumble-as-ai-rally-doubts-grip-wall-street
  relation: related
  strength: 0.643
  confidence: medium
- slug: 2026-06-11-base-rate-analysis-casts-doubt-on-ai-investment-frenzy-as-revenue-projections-hi
  relation: related
  strength: 0.6372
  confidence: medium
- slug: 2026-06-05-nasdaq-tumbles-4-2-as-strong-jobs-data-ai-spending-fears-hit-chip-stocks
  relation: related
  strength: 0.657
  confidence: medium
draft: false
related_stories:
- slug: 2026-07-17-ibm-shares-plunge-25-after-board-debated-early-profit-warning
  headline: IBM shares plunge 25% after board debated early profit warning
  publish_date: '2026-07-17'
  relation: related
  strength: 0.6595
- slug: 2026-07-07-big-tech-ai-capex-set-to-hit-168-billion-in-q2-as-meta-weighs-renting-out-spare-
  headline: Big Tech AI capex set to hit $168 billion in Q2 as Meta weighs renting
    out spare capacity
  publish_date: '2026-07-07'
  relation: related
  strength: 0.6571
- slug: 2026-05-14-alibaba-ai-cloud-revenue-climbs-38-but-heavy-investment-drags-profit-to-loss
  headline: Alibaba AI, cloud revenue climbs 38%, but heavy investment drags profit
    to loss
  publish_date: '2026-05-14'
  relation: related
  strength: 0.667
---
## IBM trims revenue forecast as customers divert budgets to AI hardware

**Subtype:** fact

- IBM lowered its full-year revenue growth forecast to a range of 4% to 5%, down from its earlier projection of more than 5%, after customers redirected budgets toward AI hardware.
- Mainframe sales fell 42% year-over-year, pulling the company's infrastructure revenue down 7% to $3.8 billion in the second quarter.
- IBM earned $2.2 billion on revenue of $17.2 billion in the quarter and still expects full-year free cash flow to increase by about $1 billion from last year.
- Chief Executive Arvind Krishna said the company is in the "early innings of a structural shift for business" but faces an extended period of AI infrastructure spending displacing traditional enterprise systems.

## Mainframe sales drop 42% after enterprise clients shift spending to servers and memory

IBM confirmed Wednesday that its full-year revenue growth will be lower than previously forecast, now expecting a range of 4% to 5% compared with the more than 5% the company had projected earlier this year. The downgrade formalized the disappointing results that IBM disclosed in a rare preliminary letter to investors on July 14 — a move that erased about $67 billion in stock market value and sent shares down 25% in their worst single-day decline, as MSI previously reported.

The company's infrastructure revenue of $3.8 billion fell 7% from a year earlier, dragged down by a 42% drop in sales of data center mainframes, which are the core of IBM's data center business. Chief Executive Arvind Krishna attributed the miss to customers redirecting late-quarter budgets toward servers, storage and memory to lock in AI-related hardware before anticipated price increases, and to a pause on some deals as clients reassessed cybersecurity spending.

The distributed infrastructure business, which includes IBM's Power server line and storage systems, saw sales grow 37% year-over-year — an all-time record — but infrastructure revenue still declined 7% on account of the mainframe weakness, according to the company's results.

"We are taking action to accelerate our revenue growth and profitability, driving productivity across the company with AI and automation, and heavily investing in commercializing innovation at speed and scale," Krishna said in a statement.

IBM earned $2.2 billion on revenue of $17.2 billion in the second quarter. The company still expects full-year free cash flow to increase by about $1 billion from last year. Shares rose as much as 3.5% in after-market trading following the release of the formal earnings report.

Chief Financial Officer James Kavanaugh told The Wall Street Journal that IBM failed to close "tens of deals" in the second quarter, which led to the disappointing results. He said the company has since closed roughly a third of those deals. Kavanaugh said the company does not expect supply chain constraints in the AI infrastructure market to improve in the near term. "Everyone in the industry is saying this will extend for a period of time," he said.

The headwinds facing IBM differ from the software-replacement fears that have weighed on companies such as Salesforce, Workday and Snowflake. IBM's warning indicated that AI infrastructure spending is now displacing hardware budgets as well, squeezing a company whose customers install traditional systems in their own data centers and can defer large-ticket purchases such as mainframe upgrades.

Kavanaugh said that despite the rough quarter, IBM's mainframe business should grow in the second half of the year. "Mainframe has always been the most secure, resilient, scalable system in the world," he said. "That's why it runs over 70% of the world's transaction volumes in terms of value."

The episode has put Krishna's leadership under scrutiny and made IBM, which employs 260,000 people, the subject of breakup and activist speculation, the Journal reported. The board of directors is expected to meet again in late July and faces pressure from an impatient Wall Street that is unlikely to tolerate several more quarters of underperformance, according to the Journal.

## Sources

### src_001 — The Wall Street Journal, national_daily, Tier 1, originating
**Author:** Anissa Gardizy
**Publication date:** 2026-07-22
**Title:** IBM Lowers Its Growth Outlook as Sales of Data Center Mainframes Sink 42%
**URL:** https://www.wsj.com/business/earnings/ibm-q2-earnings-report-2026-stock-ded1f568
