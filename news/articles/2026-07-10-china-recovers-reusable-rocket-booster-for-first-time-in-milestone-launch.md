---
headline: China recovers reusable rocket booster for first time in milestone launch
secondary_headline: Net-catch recovery shows China is catching up quickly, experts
  say
publish_date: '2026-07-10'
lede: China successfully launched and partially recovered a rocket Friday, marking
  the first time the country has demonstrated rocket-recovery technology. The Long
  March-10B launched into orbit around noon from southern Hainan province on its maiden
  flight, and minutes later a giant net mounted on a seaborne platform caught the
  descending booster, state broadcaster CCTV footage showed.
nut_graf: The achievement marks a milestone for Beijing's space program as it competes
  with the United States and Elon Musk's SpaceX. SpaceX first recovered a section
  of its Falcon 9 rocket in 2015, giving the U.S. program a cost advantage through
  reusable hardware. Beijing has pursued a yearslong drive to transform itself into
  a space power, and experts said Friday's launch shows China is catching up quickly.
primary_entities:
- China
- SpaceX
- China Aerospace Science and Technology Corp.
- Georgetown University
- Long March-10B
primary_themes:
- space technology
- reusable rockets
- U.S.-China competition
topic_tags:
- space programme
- achievement
- science and technology
- international relations
storyline_nexus: []
geographic_location: United States
floor_values_engaged:
- value: informed_citizenship
  intensity: 0.6
- value: truthfulness
  intensity: 0.6
framework_version: 1.4.1
generation_timestamp: '2026-07-10T08:35:32-07:00'
source_cluster_id: cluster_wsj_2026-07-10_cessfully-launches-reusable-rocket-in-a-
gdelt_event_ids: []
consensus_floor_version: v0.3.0
publication_mindspec_version: v0.3.0
license: https://creativecommons.org/publicdomain/zero/1.0/
ai_disclosure: This article was generated algorithmically by Main Street Independent
  from the public sources listed in its Sources section.
ai_generated: true
sources:
  count: 1
  outlets:
  - The Wall Street Journal
  outlet_classes:
  - national_daily
  highest_reliability_tier: 1
  has_originating: true
  has_primary_document: false
figures_aggregate:
  count: 0
  series_ids: []
  sources: []
cross_article_links:
- slug: 2026-05-22-spacex-launches-biggest-starship-yet-on-a-test-flight
  relation: related
  strength: 0.6539
  confidence: medium
- slug: 2026-05-23-spacex-launches-biggest-upgraded-starship-on-test-flight-to-moon-plans
  relation: related
  strength: 0.6486
  confidence: medium
- slug: 2026-05-23-spacex-launches-biggest-starship-yet-on-test-flight-aims-at-moon
  relation: related
  strength: 0.6241
  confidence: medium
- slug: 2026-07-06-china-test-launches-long-range-ballistic-missile-from-submarine-drawing-condemna
  relation: related
  strength: 0.6281
  confidence: medium
- slug: 2026-05-22-spacex-scrubs-starship-launch-thursday-will-try-again-friday
  relation: related
  strength: 0.6195
  confidence: medium
- slug: 2026-03-05-china-s-matrix-evtol-demonstrates-big-electric-flying-taxi-potential
  relation: related
  strength: 0.6146
  confidence: medium
- slug: 2026-05-17-boeing-says-china-agreed-to-buy-200-aircraft-after-trump-xi-summit
  relation: related
  strength: 0.6139
  confidence: medium
- slug: 2026-05-17-boeing-lands-200-plane-order-from-china-at-trump-xi-summit
  relation: related
  strength: 0.608
  confidence: medium
- slug: 2026-06-10-taiwan-test-fires-u-s--supplied-missile-launcher-toward-china
  relation: related
  strength: 0.6001
  confidence: medium
- slug: 2026-05-16-putin-to-visit-chinese-leader-xi-jinping-days-after-trump-s-trip-to-beijing
  relation: related
  strength: 0.5961
  confidence: medium
- slug: 2026-06-05-rocket-lab-positions-as-leading-alternative-to-spacex-for-space-investors
  relation: related
  strength: 0.5945
  confidence: medium
- slug: 2026-05-21-starship-s-redesign-launch-nears-as-spacex-faces-ipo-critical-test
  relation: related
  strength: 0.5932
  confidence: medium
- slug: 2026-06-12-spacex-ipo-tops-176-company-surpasses-2-trillion-market-cap-on-debut
  relation: related
  strength: 0.5807
  confidence: medium
- slug: 2026-06-08-spacex-deepens-ties-with-pentagon-wins-multibillion-dollar-space-force-contracts
  relation: related
  strength: 0.575
  confidence: medium
- slug: 2026-06-08-spacex-ipo-begins-trading-makes-musk-world-s-first-trillionaire
  relation: related
  strength: 0.5744
  confidence: medium
- slug: 2026-04-25-chinese-automakers-show-off-ev-and-charging-tech-at-beijing-auto-show
  relation: related
  strength: 0.5744
  confidence: medium
- slug: 2026-04-24-china-s-top-automakers-showcase-ev-and-charging-tech-at-beijing-show
  relation: related
  strength: 0.5765
  confidence: medium
- slug: 2026-04-16-china-s-economy-grows-5-in-q1-largely-shrugging-off-iran-war-shocks
  relation: related
  strength: 0.5774
  confidence: medium
- slug: 2026-04-15-china-s-economy-grows-5-in-q1-easing-iran-war-drag-concerns
  relation: related
  strength: 0.5702
  confidence: medium
- slug: 2026-05-20-spacex-plans-massive-public-share-sale-with-elon-musk-at-center
  relation: related
  strength: 0.5718
  confidence: medium
draft: false
related_stories: []
---
## China recovers reusable rocket booster for first time in milestone launch

**Subtype:** fact

- China successfully launched and partially recovered the Long March-10B rocket booster on its maiden flight Friday from Hainan province, marking the country's first demonstration of rocket-recovery technology.
- A giant net mounted on a seaborne platform caught the descending booster, representing the first net-based rocket launch recovery, according to state-owned developer China Aerospace Science and Technology Corp.
- Two previous Chinese rocket-recovery attempts in December failed.
- Beijing aims to land astronauts on the moon by 2030 and has been building a commercial space sector to challenge SpaceX.

## Net-catch recovery shows China is catching up quickly, experts say

China launched and partially recovered a rocket Friday in a milestone for Beijing's space program as it competes with the United States and Elon Musk's SpaceX.

The Long March-10B, on its maiden flight, lifted off around noon from southern Hainan province and entered orbit. Minutes later, a giant net caught the rocket's booster as it descended, state broadcaster CCTV footage showed. It was the first time China has successfully demonstrated rocket-recovery technology.

SpaceX first recovered a section of its Falcon 9 rocket in 2015, establishing a cost advantage for the U.S. space program by enabling the reuse of expensive hardware between launches. China has until now relied on disposable rockets.

The Long March-10B stands roughly 200 feet tall and carries a payload of more than 16 metric tons, state media reported. The recovery net was mounted on top of a seaborne platform and represented the first net-based rocket launch recovery, according to China Aerospace Science and Technology Corp., the state-owned firm that developed the vehicle. The company said it expected to reuse the recovered booster by the end of the year.

Two previous Chinese attempts to recover rockets in December failed.

Sam Bresnick, a research fellow who studies China at Georgetown University's Center for Security and Emerging Technology, said it would be a "big deal" if China's space program can regularly recover and reuse parts of its rockets.

"It pretty radically decreases the cost of building out these giant satellite constellations that China has made very clear that it wants to build out," Bresnick said. He added that Beijing was investing in space technology for both its economic and military applications.

China Aerospace Science and Technology Corp. said the successful launch would "lay a solid foundation for accelerating the improvement of the country's space access capabilities."

Beijing has pursued a yearslong drive to transform itself into a space power, sending more satellites into orbit and encouraging its commercial space sector to challenge SpaceX. China also aims to land astronauts on the moon by 2030.

Experts said that while China still lags behind the U.S. in space capabilities, it is catching up quickly, as shown by Friday's launch.

## Sources

### src_001 — The Wall Street Journal, national_daily, Tier 1, originating
**Author:** Katrina Northrop
**Publication date:** 2026-07-10
**Title:** China Successfully Launches Reusable Rocket in a Win for Space Program
**URL:** https://www.wsj.com/tech/china-successfully-launches-reusable-rocket-in-a-win-for-space-program-a891c5c6
