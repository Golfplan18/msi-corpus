---
headline: Microsoft surges 15.5% on AI profit signal, lifting Wall Street
secondary_headline: Bond market shows persistent inflation worries despite rally
publish_date: '2026-07-30'
lede: U.S. stocks surged Thursday as Microsoft posted its best single-day gain in
  nearly 18 years after reporting stronger-than-expected quarterly profits tied to
  growth in its artificial-intelligence business, the Associated Press reported. The
  S&P 500 rose 1.7%, the Dow Jones Industrial Average added 613 points, and the Nasdaq
  composite rallied 2.8%.
nut_graf: The rally signaled that Microsoft's heavy spending on AI is translating
  into earnings, but the bond market continued to reflect concern that inflation could
  remain elevated for years.
primary_entities:
- Microsoft Corp.
- S&P 500
- Dow Jones Industrial Average
- Nasdaq Composite
- Satya Nadella
primary_themes:
- AI spending and profitability
- Stock market rebound
- Corporate earnings
- Inflation concerns
topic_tags:
- artificial intelligence
storyline_nexus: []
storyline_continuity:
  decision: none
  selected_prior_ids: []
  evaluated_prior_ids: []
  authorized_context_prior_ids: []
geographic_location: United States
floor_values_engaged:
- value: informed_citizenship
  intensity: 0.9
- value: truthfulness
  intensity: 0.9
- value: accountability_of_power
  intensity: 0.6
framework_version: 1.5.0
generation_timestamp: '2026-07-31T12:20:26-07:00'
source_cluster_id: cluster_ap_2026-07-30_stock-markets-rates-korea-ai-oil-99b5702
gdelt_event_ids: []
consensus_floor_version: v0.3.0
publication_mindspec_version: v0.3.0
license: https://creativecommons.org/publicdomain/zero/1.0/
ai_disclosure: This article was generated algorithmically by Main Street Independent
  from the public sources listed in its Sources section.
ai_generated: true
sources:
  count: 1
  outlets:
  - Associated Press
  outlet_classes:
  - wire
  highest_reliability_tier: 1
  has_originating: true
  has_primary_document: false
figures:
- source: fred
  series_id: NASDAQCOM
  transformation: raw
  as_of: 2026-07-30
  caption: The tech-heavy Nasdaq Composite Index closed at 25,122.18 on Thursday,
    rallying 2.8% as AI enthusiasm lifted major U.S. equities.
figures_aggregate:
  count: 1
  series_ids:
  - NASDAQCOM
  sources:
  - fred
cross_article_links:
- slug: 2026-07-22-asian-shares-rise-after-ai-stocks-lead-wall-street-higher
  relation: related
  strength: 0.7344
  confidence: medium
- slug: 2026-06-08-chip-stocks-lead-wall-street-rebound-after-friday-s-selloff
  relation: related
  strength: 0.7376
  confidence: medium
- slug: 2026-06-04-dow-closes-at-record-as-healthcare-financial-stocks-lift-market-tech-lags
  relation: related
  strength: 0.718
  confidence: medium
- slug: 2026-06-02-s-p-500-notches-ninth-straight-gain-longest-streak-in-a-year
  relation: related
  strength: 0.7231
  confidence: medium
- slug: 2026-05-14-tech-stocks-push-wall-street-to-records-while-inflation-data-weighs-on-broader-m
  relation: related
  strength: 0.7573
  confidence: medium
- slug: 2026-05-14-tech-rally-lifts-wall-street-to-records-as-oil-inflation-pressure
  relation: related
  strength: 0.7544
  confidence: medium
- slug: 2026-05-21-us-stocks-recover-as-bond-pressure-eases-and-oil-gives-back-gains
  relation: related
  strength: 0.7225
  confidence: medium
- slug: 2026-05-20-us-stocks-rebound-as-yields-ease-and-oil-prices-pull-back
  relation: related
  strength: 0.7184
  confidence: medium
- slug: 2026-05-15-cisco-boosts-wall-street-records-s-p-hits-another-high-dow-reclaims-50-000
  relation: related
  strength: 0.7255
  confidence: medium
- slug: 2026-05-15-dow-back-above-50-000-for-first-time-since-iran-war-as-ai-boom-fuels-records
  relation: related
  strength: 0.7202
  confidence: medium
- slug: 2026-05-19-wall-street-rebounds-as-bond-market-pressure-eases-and-oil-retreats
  relation: related
  strength: 0.7101
  confidence: medium
- slug: 2026-02-25-u-s-stocks-rebound-after-ai-deal-optimism-offsets-earlier-selloff
  relation: related
  strength: 0.79
  confidence: medium
- slug: 2026-02-18-us-stocks-edge-higher-after-swinging-through-worries-about-ai-and-the-economy
  relation: related
  strength: 0.7678
  confidence: medium
- slug: 2026-02-08-u-s-stocks-surge-dow-tops-50-000-as-bitcoin-stabilizes
  relation: related
  strength: 0.7684
  confidence: medium
- slug: 2026-02-26-tech-stocks-lift-wall-street-nvidia-shares-rise-ahead-of-results
  relation: related
  strength: 0.7478
  confidence: medium
- slug: 2026-02-19-nvidia-boosts-stocks-as-meta-deal-lifts-s-p-500-and-nasdaq
  relation: related
  strength: 0.7434
  confidence: medium
- slug: 2026-04-01-dow-surges-1-125-points-as-market-prices-hopes-for-iran-war-end
  relation: related
  strength: 0.7116
  confidence: medium
- slug: 2026-01-30-wall-street-closes-near-where-it-began-after-microsoft-gold-swings
  relation: related
  strength: 0.7366
  confidence: medium
- slug: 2026-01-03-us-stocks-edge-higher-in-a-wobbly-start-to-2026
  relation: related
  strength: 0.7403
  confidence: medium
- slug: 2026-01-06-s-p-500-hits-record-on-third-trading-day-as-big-tech-leads-broad-gains
  relation: related
  strength: 0.7368
  confidence: medium
draft: false
related_stories:
- slug: 2026-07-31-amazon-surge-lifts-s-p-500-as-apple-drop-and-oil-concerns-weigh
  headline: Amazon surge lifts S&P 500 as Apple drop and oil concerns weigh
  publish_date: '2026-07-31'
  relation: related
  strength: 0.7739
- slug: 2026-07-31-kospi-posts-record-single-day-gain-as-semiconductor-stocks-rally
  headline: KOSPI posts record single-day gain as semiconductor stocks rally
  publish_date: '2026-07-31'
  relation: related
  strength: 0.7266
- slug: 2026-07-22-asian-shares-rise-after-ai-stocks-lead-wall-street-higher
  headline: Asian shares rise after AI stocks lead Wall Street higher
  publish_date: '2026-07-22'
  relation: related
  strength: 0.7647
- slug: 2026-06-02-s-p-500-notches-ninth-straight-gain-longest-streak-in-a-year
  headline: S&P 500 notches ninth straight gain, longest streak in a year
  publish_date: '2026-06-02'
  relation: related
  strength: 0.73
- slug: 2026-05-14-tech-rally-lifts-wall-street-to-records-as-oil-inflation-pressure
  headline: Tech rally lifts Wall Street to records as oil, inflation pressure
  publish_date: '2026-05-14'
  relation: related
  strength: 0.7571
- slug: 2026-06-02-fomo-drives-wall-street-pros-into-ai-rally-as-career-risk-outweighs-caution
  headline: FOMO drives Wall Street pros into AI rally as career risk outweighs caution
  publish_date: '2026-06-02'
  relation: related
  strength: 0.7073
- slug: 2026-05-26-another-surge-for-micron-wall-street-s-latest-1-trillion-company-sends-us-stocks
  headline: Another surge for Micron, Wall Street's latest $1 trillion company, sends
    US stocks to records
  publish_date: '2026-05-26'
  relation: related
  strength: 0.7169
- slug: 2026-05-14-tech-stocks-push-wall-street-to-records-while-inflation-data-weighs-on-broader-m
  headline: Tech stocks push Wall Street to records while inflation data weighs on
    broader market
  publish_date: '2026-05-14'
  relation: related
  strength: 0.7357
image:
  url: /articles/2026-07-30-microsoft-surges-15-5-on-ai-profit-signal-lifting-wall-street.png
  alt: 'Illustration accompanying article: Microsoft surges 15.5% on AI profit signal,
    lifting Wall Street'
  credit: Main Street Independent (algorithmic)
  source: ai_generated
  license: https://creativecommons.org/publicdomain/zero/1.0/
  disclosure: AI-generated image. Prompt summary and model identifier available in
    metadata.
  ai_model: openrouter:openai/gpt-5.4-image-2
  ai_prompt: hand-drawn, black ink linework, solid white background, fully opaque,
    linework dominant, varied pen technique | square 1:1 composition, full-bleed square
    frame that fills the entire canvas edge to edg
---
## Microsoft surges 15.5% on AI profit signal, lifting Wall Street

**Subtype:** fact

- Microsoft Corp. shares surged 15.5% in their best single-day gain since 2008 after the company reported stronger-than-expected quarterly profits, driven by growth in its Azure cloud business.
- Chief Executive Satya Nadella said the results reflect how customers are using Microsoft's platform to move into artificial intelligence.
- The S&P 500 rallied 1.7%, the Dow Jones Industrial Average added 613 points, or 1.2%, and the Nasdaq composite gained 2.8% a day after its worst drop in seven weeks.
- In the bond market, worries remained about inflation potentially remaining high for years, according to the Associated Press.

## Bond market shows persistent inflation worries despite rally

Microsoft Corp. shares surged 15.5% on Thursday in the company's best single-day gain in nearly 18 years after it reported quarterly profits that exceeded Wall Street expectations, the Associated Press reported.

The rally was driven by strong growth in Microsoft's Azure cloud business. Chief Executive Satya Nadella said the results reflect how customers are using Microsoft's platform to move into artificial intelligence, signaling that the company's heavy spending on AI is translating into earnings.

The broader market followed Microsoft higher. The S&P 500 rallied 1.7% and more than recovered its drop from the previous session, which had been the worst in seven weeks. The Dow Jones Industrial Average jumped 613 points, or 1.2%. The Nasdaq composite, which is full of artificial-intelligence stocks, gained 2.8% a day after it fell 9.8% below its record set last month.

Computer-chip companies also regained some of their sharp recent losses, the AP reported.

In the bond market, however, worries remained about inflation potentially remaining high for years, according to the AP.

## Sources

### src_001 — Associated Press, wire, Tier 1, originating
**Author:** Stan Choe
**Publication date:** 2026-07-30
**Title:** Microsoft's best day since 2008 leads US stocks, while inflation worries remain in the bond market
**URL:** https://apnews.com/article/stock-markets-rates-korea-ai-oil-99b5702d93a2b5c6e513fb952ccdcc92
