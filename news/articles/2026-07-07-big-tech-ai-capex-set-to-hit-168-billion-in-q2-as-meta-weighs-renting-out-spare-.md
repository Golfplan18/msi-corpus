---
headline: Big Tech AI capex set to hit $168 billion in Q2 as Meta weighs renting out
  spare capacity
secondary_headline: SpaceX xAI and Anthropic signed $1.25 billion monthly compute
  deal before IPO
publish_date: '2026-07-07'
lede: The four largest technology companies are on track to spend $168 billion on
  artificial intelligence infrastructure in the second quarter, a 74% surge from a
  year earlier, according to Wall Street analyst estimates cited by The Wall Street
  Journal, after Meta CEO Mark Zuckerberg said in May his company may have overbuilt
  its computing capacity and was considering renting out excess capacity through a
  new cloud business.
primary_entities:
- Google
- Microsoft
- Amazon
- Meta Platforms
- SpaceX
- Anthropic
- Nvidia
- Broadcom
- Advanced Micro Devices
- Intel
- SK Hynix
- Micron Technology
- Caterpillar
primary_themes:
- AI infrastructure spending
- technology investment cycle
- semiconductor industry
- market volatility
- corporate strategy
topic_tags:
- artificial intelligence
- economy, business and finance
- computing and information technology
- technology and engineering
storyline_nexus: []
geographic_location: United States
floor_values_engaged:
- value: accountability_of_power
  intensity: 0.9
- value: informed_citizenship
  intensity: 0.6
- value: truthfulness
  intensity: 0.6
framework_version: 1.4.1
generation_timestamp: '2026-07-07T02:46:08-07:00'
source_cluster_id: cluster_wsj_2026-07-07_tech-ai-will-someone-finally-blink-in-th
gdelt_event_ids: []
consensus_floor_version: v0.3.0
publication_mindspec_version: v0.3.0
license: https://creativecommons.org/publicdomain/zero/1.0/
ai_disclosure: This article was generated algorithmically by Main Street Independent
  from the public sources listed in its Sources section.
ai_generated: true
sources:
  count: 1
  outlets:
  - The Wall Street Journal
  outlet_classes:
  - national_daily
  highest_reliability_tier: 1
  has_originating: true
  has_primary_document: false
figures_aggregate:
  count: 0
  series_ids: []
  sources: []
cross_article_links:
- slug: 2026-07-03-ai-data-centers-consume-as-much-as-12-times-more-water-indirectly-than-directly-
  relation: related
  strength: 0.6828
  confidence: medium
- slug: 2026-06-27-ai-bubble-warnings-mount-as-dot-com-parallels-spread
  relation: related
  strength: 0.7107
  confidence: medium
- slug: 2026-06-24-amazon-and-google-lead-ai-power-race-but-strategies-diverge
  relation: related
  strength: 0.7108
  confidence: medium
- slug: 2026-06-23-tech-shares-tumble-as-ai-rally-doubts-grip-wall-street
  relation: related
  strength: 0.6736
  confidence: medium
- slug: 2026-06-23-oracle-cuts-21-000-roles-as-it-pivots-to-ai-annual-report-shows
  relation: related
  strength: 0.6726
  confidence: medium
- slug: 2026-06-09-wall-street-funnels-record-capital-to-ai-build-out-across-equity-and-debt-market
  relation: related
  strength: 0.7428
  confidence: medium
- slug: 2026-06-11-base-rate-analysis-casts-doubt-on-ai-investment-frenzy-as-revenue-projections-hi
  relation: related
  strength: 0.7214
  confidence: medium
- slug: 2026-06-12-spacex-ipo-and-pending-ai-listings-bind-us-retirement-funds-to-tech-sector-analy
  relation: related
  strength: 0.7038
  confidence: medium
- slug: 2026-06-11-tech-market-roundup-oracle-ai-push-tsmc-leadership-crypto-slump-super-micro-dilu
  relation: related
  strength: 0.6796
  confidence: medium
- slug: 2026-06-10-tech-stocks-slide-bitcoin-slumps-below-61k-as-market-selloff-deepens
  relation: related
  strength: 0.6779
  confidence: medium
- slug: 2026-06-04-leading-ai-companies-push-toward-blockbuster-ipos-amid-cash-race
  relation: related
  strength: 0.6928
  confidence: medium
- slug: 2026-06-02-ai-memory-chip-supply-crunch-deepens-easyjet-jumps-on-takeover-speculation-bitco
  relation: related
  strength: 0.6943
  confidence: medium
- slug: 2026-06-01-alphabet-plans-80-billion-equity-raise-to-fund-ai-infrastructure-expansion
  relation: related
  strength: 0.6851
  confidence: medium
- slug: 2026-05-20-spacex-ipo-plans-could-reshape-markets-as-spending-shifts-to-rockets-and-ai
  relation: related
  strength: 0.7156
  confidence: medium
- slug: 2026-04-30-meta-beats-revenue-expectations-raises-2026-capital-spending-forecast
  relation: related
  strength: 0.7532
  confidence: medium
- slug: 2026-05-15-companies-from-cisco-to-meta-cite-ai-as-layoffs-mount
  relation: related
  strength: 0.7102
  confidence: medium
- slug: 2026-05-15-streamline-operations-how-ai-is-fueling-tech-world-layoffs
  relation: related
  strength: 0.6989
  confidence: medium
- slug: 2026-05-14-alibaba-ai-cloud-revenue-climbs-38-but-heavy-investment-drags-profit-to-loss
  relation: related
  strength: 0.6887
  confidence: medium
- slug: 2026-04-30-alphabet-first-quarter-profit-jumps-as-google-s-ai-bets-pay-off
  relation: related
  strength: 0.6983
  confidence: medium
- slug: 2026-04-25-meta-cuts-8-000-jobs-as-ai-spending-surges-and-efficiency-drive-accelerates
  relation: related
  strength: 0.7056
  confidence: medium
draft: false
image:
  url: /articles/2026-07-07-big-tech-ai-capex-set-to-hit-168-billion-in-q2-as-meta-weighs-renting-out-spare-.png
  alt: 'Illustration accompanying article: Big Tech AI capex set to hit $168 billion
    in Q2 as Meta weighs renting out spare capacity'
  credit: Main Street Independent (algorithmic)
  source: ai_generated
  license: https://creativecommons.org/publicdomain/zero/1.0/
  disclosure: AI-generated image. Prompt summary and model identifier available in
    metadata.
  ai_model: openrouter:openai/gpt-5.4-image-2
  ai_prompt: hand-drawn, black ink linework, solid white background, fully opaque,
    linework dominant, varied pen technique | square 1:1 composition, full-bleed square
    frame that fills the entire canvas edge to edg
related_stories:
- slug: 2026-07-03-ai-data-centers-consume-as-much-as-12-times-more-water-indirectly-than-directly-
  headline: AI data centers consume as much as 12 times more water indirectly than
    directly, lab finds
  publish_date: '2026-07-03'
  relation: related
  strength: 0.6691
- slug: 2026-06-27-ai-bubble-warnings-mount-as-dot-com-parallels-spread
  headline: AI bubble warnings mount as dot-com parallels spread
  publish_date: '2026-06-27'
  relation: related
  strength: 0.7059
- slug: 2026-06-09-wall-street-funnels-record-capital-to-ai-build-out-across-equity-and-debt-market
  headline: Wall Street funnels record capital to AI build-out across equity and debt
    markets
  publish_date: '2026-06-09'
  relation: related
  strength: 0.7708
- slug: 2026-06-24-amazon-and-google-lead-ai-power-race-but-strategies-diverge
  headline: Amazon and Google Lead AI Power Race, but Strategies Diverge
  publish_date: '2026-06-24'
  relation: related
  strength: 0.687
- slug: 2026-06-23-tech-shares-tumble-as-ai-rally-doubts-grip-wall-street
  headline: Tech shares tumble as AI rally doubts grip Wall Street
  publish_date: '2026-06-23'
  relation: related
  strength: 0.6852
- slug: 2026-06-11-base-rate-analysis-casts-doubt-on-ai-investment-frenzy-as-revenue-projections-hi
  headline: Base-rate analysis casts doubt on AI investment frenzy as revenue projections
    hit historic extremes
  publish_date: '2026-06-11'
  relation: related
  strength: 0.708
- slug: 2026-06-12-spacex-ipo-and-pending-ai-listings-bind-us-retirement-funds-to-tech-sector-analy
  headline: SpaceX IPO and pending AI listings bind US retirement funds to tech sector,
    analysis says
  publish_date: '2026-06-12'
  relation: related
  strength: 0.6913
- slug: 2026-06-01-alphabet-plans-80-billion-equity-raise-to-fund-ai-infrastructure-expansion
  headline: Alphabet plans $80 billion equity raise to fund AI infrastructure expansion
  publish_date: '2026-06-01'
  relation: related
  strength: 0.721
analyses:
- '2026-07-07-financial-coverage-frames-hyperscaler-artificial-intelligence-capital-expenditur'
---
## Big Tech AI capex set to hit $168 billion in Q2 as Meta weighs renting out spare capacity

**Subtype:** fact

- The four largest technology companies—Google, Microsoft, Amazon, and Meta—spent an estimated $168 billion on AI infrastructure in the second quarter of 2026, a 74% increase from a year earlier, according to consensus estimates from Visible Alpha cited by The Wall Street Journal.
- Meta is developing a cloud-computing business to rent out excess AI computing capacity, a move that CEO Mark Zuckerberg told investors in late May was an option if the company determines it has overbuilt, Bloomberg reported.
- AI-exposed stocks sold off following the report; the PHLX Semiconductor Index fell 11% over two days, with Nvidia, Broadcom, Advanced Micro Devices, and Intel all declining.
- SpaceX's xAI division signed a $1.25 billion monthly compute-sharing deal with Anthropic before SpaceX's initial public offering last month, the Journal reported.

## SpaceX xAI and Anthropic signed $1.25 billion monthly compute deal before IPO

Combined capital spending by Google, Microsoft, Amazon, and Meta surged 74% year over year to an estimated $168 billion in the quarter ending June, according to The Wall Street Journal, citing Visible Alpha consensus estimates. The spending has crimped free cash flow and weighed on stock prices; only Alphabet has outperformed the S&P 500 this year among the four.

Meta has been the most aggressive in its AI investments, with founder and Chief Executive Mark Zuckerberg building a division called Meta Superintelligence Labs. The company expects to spend well over half its revenue on capital investments this year, which could push free cash flow negative for the first time as a public company, according to the Journal. Bloomberg reported last week that Meta is developing a cloud-computing business using the extensive AI network it has built. Zuckerberg told investors at the company's annual shareholder meeting in late May that renting out capacity is an option if Meta determines it has overbuilt. "We haven't done that yet because we think that we have a use for the compute," Zuckerberg said. "But obviously, if we get to a point where we feel that we have overbuilt, then that is an option that we have."

The report of Meta's potential cloud service triggered a broad selloff in AI-exposed stocks. The PHLX Semiconductor Index slid 11% over a two-day period, according to the Journal. Major chip makers including Nvidia, Broadcom, Advanced Micro Devices, and Intel all fell. Memory-chip makers SK Hynix and Micron each lost 17% and 15%, respectively, in that timeframe. Caterpillar, whose generators power data centers, shed 10% over two days. The Nasdaq composite index fell nearly 2% over the two-day period.

Not all signs point to a pullback. Before SpaceX went public last month, its xAI business signed a major deal to share computing capacity with Anthropic for $1.25 billion a month, the Journal reported. SpaceX itself spent $12.7 billion in capital expenditures on its AI division last year—triple its rocket-business spending—and analysts expect more than $37 billion in AI capex this year, per Visible Alpha.

Analysts remain divided on whether Meta intends to scale back. Jefferies analyst Brent Thill wrote that "Meta is not stepping away from the AI race; it is turning early, aggressive capacity commitments into a strategic value creation option." Justin Patterson of KeyBanc Capital said "it is conceivable that the scope of MSL's ambitions have narrowed vs. Meta's original AI goals when it began the capex cycle," the Journal reported.

Chip companies alone now make up about 18% of the S&P 500's total market capitalization, compared with about 5% five years ago, according to S&P Global Market Intelligence data cited by the Journal. The four hyperscale companies' combined capital expenditures are projected to reach $710 billion this year and could hit $1 trillion in 2027, though the growth rate would be roughly half of this year's pace.

## Sources

### src_001 — The Wall Street Journal, national_daily, Tier 1, originating
**Author:** Dan Gallagher
**Publication date:** 2026-07-07
**Title:** Will Someone Finally Blink in the AI Spending War?
**URL:** https://www.wsj.com/tech/ai/will-someone-finally-blink-in-the-ai-spending-war-0f59aa60

---

<!-- analysis-link:2026-07-07-financial-coverage-frames-hyperscaler-artificial-intelligence-capital-expenditur -->
**Going deeper:** [Read MSI's analysis of hyperscaler ai capex framing →](/analyses/2026-07-07-financial-coverage-frames-hyperscaler-artificial-intelligence-capital-expenditur)
