---
headline: Taiwan considers tighter export controls on AI chips bound for China
publish_date: '2026-06-10'
lede: Taiwan is reviewing plans to tighten export controls on advanced artificial
  intelligence chips bound for China, a move that would align the island's restrictions
  more closely with existing U.S. regulations and could draw sharp opposition from
  Beijing, according to a report Wednesday from Asia Today.
nut_graf: The proposal would expand Taiwan's sales restrictions beyond blacklisted
  companies such as Huawei to cover all customers in China, giving authorities legal
  tools to treat the smuggling of AI chips as a criminal offense. If adopted, the
  rules would affect major Taiwanese server assemblers including Gigabyte and Asus.
primary_entities:
- Taiwan
- China
- United States
- Nvidia
- Gigabyte
- Asus
- Huawei
primary_themes:
- semiconductor export controls
- U.S.-China technology rivalry
- Taiwan security
topic_tags:
- artificial intelligence
- computing and information technology
- international trade
- government policy
storyline_nexus:
- t
- e
- c
- h
- n
- o
- l
- o
- g
- y
- '-'
- r
- e
- g
- u
- l
- a
- t
- o
- r
- y
geographic_location: United States
floor_values_engaged: []
framework_version: 1.3.0
generation_timestamp: '2026-06-11T00:02:31Z'
source_cluster_id: cluster_upi_2026-06-11_-news-2026-06-10-taiwan-ai-chip-export-c
gdelt_event_ids: []
consensus_floor_version: 1.0.0
publication_mindspec_version: 1.0.0
license: CC0-1.0
ai_generated: true
sources:
  count: 1
  outlets:
  - United Press International
  outlet_classes:
  - wire
  highest_reliability_tier: 2
  has_originating: true
  has_primary_document: false
figures_aggregate:
  count: 0
  series_ids: []
  sources: []
cross_article_links:
- slug: 2026-03-20-3-men-charged-in-smuggling-scheme-to-divert-nvidia-ai-chips-to-china
  relation: related
  strength: 0.7177
  confidence: medium
- slug: 2026-02-11-ai-boom-lifts-taiwan-chip-economy-but-investors-worry-about-bubble-and-risk
  relation: related
  strength: 0.7498
  confidence: medium
- slug: 2026-06-01-china-tightens-outbound-investment-rules-amid-u-s--china-tech-rivalry
  relation: related
  strength: 0.7041
  confidence: medium
- slug: 2026-06-08-pentagon-adds-alibaba-baidu-byd-to-list-of-chinese-military-linked-companies
  relation: related
  strength: 0.6453
  confidence: medium
- slug: 2026-05-13-what-to-know-about-trump-xi-summit-in-beijing
  relation: related
  strength: 0.678
  confidence: medium
- slug: 2026-04-24-trump-administration-targets-foreign-exploitation-of-u-s-ai-models
  relation: related
  strength: 0.6648
  confidence: medium
- slug: 2026-05-05-u-s-allowed-silicon-valley-surveillance-tech-sales-to-china-ap-finds
  relation: related
  strength: 0.6638
  confidence: medium
draft: false
related_stories:
- slug: 2026-06-01-nvidia-unveils-rtx-spark-chip-for-ai-powered-personal-computers
  headline: Nvidia unveils RTX Spark chip for AI-powered personal computers
  publish_date: '2026-06-01'
  relation: related
  strength: 0.6967
- slug: 2026-06-02-huawei-s-own-paper-shows-chip-gap-persists-despite-tau-law-claim-analysts-say
  headline: Huawei’s own paper shows chip gap persists despite Tau Law claim, analysts
    say
  publish_date: '2026-06-02'
  relation: related
  strength: 0.6748
- slug: 2026-06-01-china-tightens-outbound-investment-rules-amid-u-s--china-tech-rivalry
  headline: China tightens outbound investment rules amid U.S.-China tech rivalry
  publish_date: '2026-06-01'
  relation: related
  strength: 0.679
- slug: 2026-05-16-trump-calls-taiwan-a-negotiating-chip-spurring-taiwan-and-china-anxieties
  headline: Trump calls Taiwan a “negotiating chip,” spurring Taiwan and China anxieties
  publish_date: '2026-05-16'
  relation: related
  strength: 0.6935
- slug: 2026-05-16-trump-calls-taiwan-arms-sales-a-very-good-negotiating-chip-with-china
  headline: Trump calls Taiwan arms sales a 'very good negotiating chip' with China
  publish_date: '2026-05-16'
  relation: related
  strength: 0.676
- slug: 2026-05-13-what-to-know-about-trump-xi-summit-in-beijing
  headline: What to know about Trump-Xi summit in Beijing
  publish_date: '2026-05-13'
  relation: related
  strength: 0.6683
- slug: 2026-05-10-trump-and-xi-summit-puts-taiwan-at-center-amid-arms-and-chips-debate
  headline: Trump and Xi summit puts Taiwan at center amid arms and chips debate
  publish_date: '2026-05-10'
  relation: related
  strength: 0.6607
- slug: 2026-05-05-u-s-allowed-silicon-valley-surveillance-tech-sales-to-china-ap-finds
  headline: U.S. allowed Silicon Valley surveillance tech sales to China, AP finds
  publish_date: '2026-05-05'
  relation: related
  strength: 0.668
---
## Summary

**Subtype:** fact

- Taiwan is considering expanding export controls on advanced AI chips to cover all customers in China, not only blacklisted companies like Huawei, according to people familiar with the matter cited by Bloomberg.
- The proposal would give Taiwanese authorities legal tools to treat the smuggling of AI chips to China as a criminal offense, mirroring the U.S. enforcement approach.
- Taiwanese companies assembling Nvidia-based servers, including Gigabyte and Asus, could face additional compliance burdens under the new restrictions.
- China is expected to strongly oppose the measure, having previously accused Taiwan's government of damaging the island's interests by aligning with Washington.

Taiwan is reviewing plans to expand export controls on advanced artificial intelligence chips bound for China, a move that would bring the island's regulations more closely in line with U.S. restrictions on the technology, Bloomberg reported Wednesday, citing people familiar with the matter.

The measure is intended to give Taiwanese authorities additional legal tools to stop advanced hardware — including AI servers equipped with Nvidia chips — from being routed through Taiwan and into China, according to the report. The United States has restricted exports of advanced AI chips to China since 2022, requiring U.S. companies to obtain government approval before selling certain products to Chinese customers. Washington has said the rules are intended to prevent China from using advanced Nvidia processors to gain a military advantage.

Taiwanese officials are reviewing a broader AI semiconductor control plan as part of ongoing trade talks with the United States, Bloomberg reported. The proposal would expand sales restrictions beyond specific blacklisted companies, such as Huawei, to cover all customers in China. If implemented, Taiwan would be able to treat the smuggling of AI chips into China as a criminal offense, similar to the U.S. approach.

The proposed rules could place additional compliance burdens on Taiwanese companies including Gigabyte and Asus, which assemble Nvidia-based servers. Those firms may need to strengthen business management and monitoring systems to comply with the new restrictions, the report said.

Taiwan's Ministry of Economic Affairs said it will continue strengthening oversight of strategic high-tech items to better align with international export-control regimes. The ministry said Taiwan and the United States are continuing consultations on issues including whether to include advanced semiconductors among regulated items.

China is expected to strongly oppose any new restrictions. Last year, after Taiwan added Huawei and Semiconductor Manufacturing International Corp., China's largest chipmaker, to its export-control blacklist, China's Foreign Ministry accused the Democratic Progressive Party government led by Taiwan President Lai Ching-te of damaging Taiwan's interests by aligning with Washington. MSI previously reported that three men were charged in March in a smuggling scheme to divert Nvidia AI chips to China, highlighting enforcement challenges on the existing export-control framework.

## Sources

### src_001 — United Press International, wire, Tier 2, originating
**Publication date:** 2026-06-10
**Title:** Taiwan weighs tighter rules for AI chip exports to China
**URL:** https://www.upi.com/Top_News/World-News/2026/06/10/taiwan-ai-chip-export-controls-on-china/1771781134484/
