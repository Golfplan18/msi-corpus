---
headline: FAA clears SpaceX Starship for test flight after mishap probe
secondary_headline: Launch could come as soon as Thursday from Texas
publish_date: '2026-07-13'
lede: The Federal Aviation Administration closed its investigation into a SpaceX Starship
  mishap Monday and cleared the company to launch the newest version of the rocket
  as soon as Thursday, the company said.
nut_graf: The flight would be the first test of Starship "V3," a redesigned version
  aimed at deploying bigger satellites, including 20 V3 Starlink satellites that would
  attempt to connect with the larger constellation via high-capacity lasers. The launch
  comes after a May test mission was postponed when the booster did not perform as
  expected, triggering the FAA review.
primary_entities:
- SpaceX
- Federal Aviation Administration
- Starship
primary_themes:
- space exploration
- aerospace regulation
- satellite technology
topic_tags:
- space programme
- transport
storyline_nexus:
- space-exploration-commercial
geographic_location: United States
floor_values_engaged:
- value: accountability_of_power
  intensity: 0.9
- value: informed_citizenship
  intensity: 0.6
- value: human_life_and_dignity
  intensity: 0.3
framework_version: 1.4.1
generation_timestamp: '2026-07-13T10:17:10-07:00'
source_cluster_id: cluster_wsj_2026-07-13_siness-airlines-spacex-plans-starship-te
gdelt_event_ids: []
consensus_floor_version: v0.3.0
publication_mindspec_version: v0.3.0
license: https://creativecommons.org/publicdomain/zero/1.0/
ai_disclosure: This article was generated algorithmically by Main Street Independent
  from the public sources listed in its Sources section.
ai_generated: true
sources:
  count: 1
  outlets:
  - The Wall Street Journal
  outlet_classes:
  - national_daily
  highest_reliability_tier: 1
  has_originating: true
  has_primary_document: false
figures_aggregate:
  count: 0
  series_ids: []
  sources: []
cross_article_links:
- slug: 2026-05-22-spacex-scrubs-starship-launch-thursday-will-try-again-friday
  relation: related
  confidence: medium
- slug: 2026-05-21-starship-s-redesign-launch-nears-as-spacex-faces-ipo-critical-test
  relation: related
  confidence: medium
- slug: 2026-05-22-spacex-launches-biggest-starship-yet-on-a-test-flight
  relation: related
  confidence: medium
- slug: 2026-05-23-spacex-launches-biggest-upgraded-starship-on-test-flight-to-moon-plans
  relation: related
  confidence: medium
- slug: 2026-05-23-spacex-launches-biggest-starship-yet-on-test-flight-aims-at-moon
  relation: related
  confidence: medium
- slug: 2026-06-12-spacex-ipo-tops-176-company-surpasses-2-trillion-market-cap-on-debut
  relation: related
  confidence: medium
- slug: 2026-07-02-spacex-starlink-plans-mobile-service-to-target-at-t-verizon
  relation: related
  confidence: medium
- slug: 2026-07-02-nasa-oig-report-warns-boeing-starliner-future-uncertain
  relation: related
  confidence: medium
- slug: 2026-05-23-spacex-launches-biggest-starship-yet-on-test-flight-aims-at-moon
  relation: related
  confidence: medium
- slug: 2026-06-05-s-p-500-declines-fast-track-for-spacex-ipo-dimming-forced-buying-boost
  relation: related
  confidence: medium
- slug: 2026-06-09-nasa-names-artemis-iii-crew-for-orbital-test-after-moon-landing-delay
  relation: related
  confidence: medium
- slug: 2026-06-04-spacex-significantly-overvalued-as-1-78tn-ipo-approaches
  relation: related
  confidence: medium
- slug: 2026-05-21-spacex-reveals-plans-for-what-could-be-the-biggest-ever-public-stock-sale
  relation: related
  confidence: medium
- slug: 2026-05-20-spacex-plans-massive-public-share-sale-with-elon-musk-at-center
  relation: related
  confidence: medium
- slug: 2026-06-05-rocket-lab-positions-as-leading-alternative-to-spacex-for-space-investors
  relation: related
  confidence: medium
- slug: 2026-05-20-spacex-ipo-plans-could-reshape-markets-as-spending-shifts-to-rockets-and-ai
  relation: related
  confidence: medium
- slug: 2026-06-03-spacex-sets-ipo-share-price-at-135-valuation-at-1-75tn
  relation: related
  confidence: medium
- slug: 2026-05-22-spacex-reveals-plans-for-biggest-ever-public-stock-sale-by-musk
  relation: related
  confidence: medium
- slug: 2026-05-21-inside-spacex-s-ipo-filing-75b-raise-lockups-and-a-1m-person-mars-goal
  relation: related
  confidence: medium
- slug: 2026-05-22-spacex-ipo-prospectus-details-losses-lockups-and-a-1-million-person-mars-goal
  relation: related
  confidence: medium
- slug: 2026-04-02-spacex-files-preliminary-paperwork-for-public-share-sale-value-could-top-1-5t
  relation: related
  confidence: medium
draft: false
related_stories:
- slug: 2026-05-22-spacex-scrubs-starship-launch-thursday-will-try-again-friday
  headline: SpaceX scrubs Starship launch Thursday, will try again Friday
  publish_date: '2026-05-22'
  relation: related
  strength: 0.7758
- slug: 2026-05-22-spacex-launches-biggest-starship-yet-on-a-test-flight
  headline: SpaceX launches biggest Starship yet on a test flight
  publish_date: '2026-05-22'
  relation: related
  strength: 0.7691
- slug: 2026-05-21-starship-s-redesign-launch-nears-as-spacex-faces-ipo-critical-test
  headline: Starship’s redesign launch nears as SpaceX faces IPO-critical test
  publish_date: '2026-05-21'
  relation: related
  strength: 0.7644
- slug: 2026-05-23-spacex-launches-biggest-upgraded-starship-on-test-flight-to-moon-plans
  headline: SpaceX launches biggest, upgraded Starship on test flight to moon plans
  publish_date: '2026-05-23'
  relation: related
  strength: 0.7528
- slug: 2026-05-23-spacex-launches-biggest-starship-yet-on-test-flight-aims-at-moon
  headline: SpaceX launches biggest Starship yet on test flight, aims at moon
  publish_date: '2026-05-23'
  relation: related
  strength: 0.7261
- slug: 2026-03-15-nasa-clears-artemis-moon-rocket-for-april-launch-with-four-astronauts
  headline: NASA clears Artemis moon rocket for April launch with four astronauts
  publish_date: '2026-03-15'
  relation: related
  strength: 0.669
---
## FAA clears SpaceX Starship for test flight after mishap probe

**Subtype:** fact

- The FAA closed its investigation into a SpaceX Starship mishap Monday and accepted the company's findings on root causes, clearing the way for the next test flight.
- The company said it is preparing to launch as soon as Thursday, with an evening launch window, for the first flight of the redesigned Starship "V3."
- The mission will carry 20 V3 Starlink satellites that will attempt to link with the broader constellation using high-capacity lasers.

## Launch could come as soon as Thursday from Texas

The Federal Aviation Administration announced the close of its investigation Monday after SpaceX delayed a May test mission when the booster did not perform as expected. The regulator said it oversaw and accepted SpaceX's findings, which identified heat effects on propulsion system components during ascent and erroneous engine alarm system settings as the two most probable root causes for the earlier problems. The FAA also accepted four corrective actions from SpaceX, including vehicle hardware and software configuration updates.

The company said Monday it is preparing to launch the flight test as soon as Thursday, with a launch window set to open in the evening. The mission would be the first flight for Starship "V3," a redesigned version of the vehicle aimed at deploying bigger satellites, according to the Wall Street Journal.

SpaceX said the launch will mark the first time Starship will carry V3 Starlink satellites to space, with the rocket expected to deploy 20 satellites that will attempt to connect with the larger Starlink constellation using high-capacity lasers.

## Sources

### src_001 — The Wall Street Journal, national_daily, Tier 1, originating
**Author:** Kelly Cloonan
**Publication date:** 2026-07-13
**Title:** SpaceX Plans Starship Test Flight After Delay
**URL:** https://www.wsj.com/business/airlines/spacex-plans-starship-test-flight-after-delay-89341569
