---
headline: Monopoly maker's bid to build in US shows limits of reshoring
secondary_headline: Nearly 80% of U.S. toys still made in China
publish_date: '2026-07-12'
lede: The WS Game Company, which produces most of its board games in China, set out
  to manufacture a special edition of Monopoly in the United States after President
  Trump's tariffs added a seven-figure bill to last year's import costs. Silva managed
  to assemble the game domestically — but it took more than a year, cost at least
  double what Chinese production would have, and still required imported dice because
  no U.S. manufacturer could be found.
nut_graf: 'The company''s struggle illustrates a core challenge of efforts to move
  manufacturing back to the U.S. decades after production shifted overseas: the country
  lacks not just the factories but the entire supplier ecosystem that makes low-margin
  consumer goods profitable. The lesson comes as the toy industry, which produces
  roughly 80% of all toys and games sold domestically in China, pushes for a tariff
  carve-out from the new U.S.-China Board of Trade.'
primary_entities:
- WS Game Company
- Jonathan Silva
- The Toy Association
- Greg Ahearn
- Stateline Industries
- Pioneer Packaging
- U.S.-China Board of Trade
primary_themes:
- US manufacturing
- tariffs and trade
- supply chain
- consumer goods
topic_tags:
- manufacturing and engineering
- international trade
- economy, business and finance
- consumer goods
storyline_nexus: []
geographic_location: United States
floor_values_engaged:
- value: accountability_of_power
  intensity: 0.6
- value: informed_citizenship
  intensity: 0.6
framework_version: 1.4.1
generation_timestamp: '2026-07-12T02:53:06-07:00'
source_cluster_id: cluster_npr_2026-07-12_nx-s1-5887378-monopoly-tariffs-china-manufacturing
gdelt_event_ids: []
consensus_floor_version: v0.3.0
publication_mindspec_version: v0.3.0
license: https://creativecommons.org/publicdomain/zero/1.0/
ai_disclosure: This article was generated algorithmically by Main Street Independent
  from the public sources listed in its Sources section.
ai_generated: true
sources:
  count: 1
  outlets:
  - NPR
  outlet_classes:
  - public_broadcaster
  highest_reliability_tier: 1
  has_originating: true
  has_primary_document: false
figures_aggregate:
  count: 0
  series_ids: []
  sources: []
cross_article_links:
- slug: 2026-03-19-how-trump-s-tariffs-have-hurt-u-s-manufacturers-instead-of-helping-them
  relation: related
  confidence: medium
- slug: 2026-02-21-businesses-brace-for-uncertain-trade-policy-after-supreme-court-tariff-ruling
  relation: related
  confidence: medium
- slug: 2026-06-06-trump-s-china-trade-war-strategy-flawed-economist-eduardo-porter-says
  relation: related
  confidence: medium
- slug: 2026-05-14-trump-and-xi-meet-in-beijing-to-stabilize-us-china-trade-after-tariff-war
  relation: related
  confidence: medium
- slug: 2026-06-14-for-us-small-businesses-trump-s-tariffs-become-a-non-issue-consultant-says
  relation: related
  confidence: medium
- slug: 2026-02-10-tariffs-boost-maine-softwood-mill-while-others-brace-for-uncertainty
  relation: related
  confidence: medium
draft: false
related_stories: []
image:
  url: /articles/2026-07-12-monopoly-maker-s-bid-to-build-in-us-shows-limits-of-reshoring.png
  alt: "Illustration accompanying article: Monopoly maker's bid to build in US shows limits of reshoring"
  credit: Main Street Independent (algorithmic)
  source: ai_generated
  license: "https://creativecommons.org/publicdomain/zero/1.0/"
  disclosure: AI-generated image. Prompt summary and model identifier available in metadata.
  ai_model: "openrouter:openai/gpt-5.4-image-2"
  ai_prompt: "hand-drawn, black ink linework, solid white background, fully opaque, linework dominant, varied pen technique | square 1:1 composition, full-bleed square frame that fills the entire canvas edge to edg"
analyses:
- '2026-07-12-ws-game-company-reshoring-effort-reveals-structural-limits-of-domestic-component'
---
## Monopoly maker's bid to build in US shows limits of reshoring

**Subtype:** fact

- WS Game Company CEO Jonathan Silva attempted to produce a special edition Monopoly game in the U.S. after tariffs added a seven-figure bill to import costs.
- The experiment took over a year to organize and cost at least double what Chinese production would have, and Silva could not find any U.S. company to manufacture dice.
- The Toy Association president Greg Ahearn said the industry is pushing for a tariff carve-out from the new U.S.-China Board of Trade, arguing toys' low margins make domestic production impractical.
- Nearly 80% of all toys and games sold in the United States are made in China, according to the toy industry trade group.

## Nearly 80% of U.S. toys still made in China

## WS Game Company's made-in-America experiment nearly fails to pass go

WS Game Company CEO Jonathan Silva decided to test whether a profitable board game could be made in the United States after he received a seven-figure tariff bill last year on games manufactured in China, where the company produces most of its high-end products.

He chose to produce a special edition of Monopoly themed to the country's 250th birthday. But the experiment immediately hit a problem: no U.S. company was willing or able to make 10,000 dice.

"We turned over every single leaf trying to find someone who would make 10,000 dice for us in the U.S.," Silva said in an interview with NPR. "It requires special machinery. It requires investment. And that type of stuff just can't happen on a random Tuesday and be ready in a couple of months."

Silva ultimately had to settle for imported dice. He was able to find domestic suppliers for the rest of the game's components — a former Hasbro factory in Massachusetts prints the Monopoly board, Pioneer Packaging makes the money tray, and a business in Indiana, Stateline Industries, fabricated custom metal tokens with shapes including a cowboy hat, covered wagon and apple pie.

But assembling all those suppliers took more than a year, causing Silva to miss the first half of the 250th birthday selling season. The total manufacturing cost for the games, which retail at $80, was at least double what production in China would have cost.

"When I place a purchase order in China, they have all those capabilities under one roof," Silva said. "For one item, it took up way too much of our resources and time to bring it to market."

Nearly 80% of all toys and games sold in the United States are made in China, according to the Toy Association, an industry trade group. The country has spent decades developing an integrated factory ecosystem that can supply both finished products and every specialized component.

Greg Ahearn, the Toy Association's president and CEO, said moving production back to the U.S. is not as straightforward as it sounds.

"That's why the re-shoring and the looking at bringing it back into the U.S. or even looking at other countries and moving it is not as easy as it sounds," Ahearn said.

Ahearn argued that while it makes sense for the United States to manufacture some strategically important products, toys and games — which carry low prices and low profit margins — are not likely candidates.

"Even if you could, who in their right mind would take their capital and invest it into creating a toy manufacturing plant?" Ahearn said. "Of all the things you could pick, we'd probably be pretty low on that list."

The toy industry is instead lobbying for a tariff carve-out. The new U.S.-China Board of Trade is considering allowing up to $30 billion worth of Chinese products to enter the U.S. tariff-free, but toys are competing with shoes, apparel and many other products for the tax break.

As MSI previously reported, businesses across multiple industries have struggled with the uncertainty created by trade policy. Silva is now awaiting a shipment from China of about $6 million worth of games for the upcoming holiday season, with no firm idea of what the tariff bill will be.

"We're really good at a lot of great things here in America," Silva said. "But we're not really great at making certain items that are consumable goods. And that's OK."

## Sources

### src_001 — NPR, public_broadcaster, Tier 1, originating
**Author:** Scott Horsley
**Publication date:** 2026-07-12
**Title:** What a Monopoly importer learned when it tried to make things in the U.S.A.
**URL:** https://www.npr.org/2026/07/12/nx-s1-5887378/monopoly-tariffs-china-manufacturing-made-inusa

---

<!-- analysis-link:2026-07-12-ws-game-company-reshoring-effort-reveals-structural-limits-of-domestic-component -->
**Going deeper:** [Read MSI's analysis of reshoring limits in toy manufacturing →](/analyses/2026-07-12-ws-game-company-reshoring-effort-reveals-structural-limits-of-domestic-component)
