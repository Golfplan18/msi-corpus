---
headline: OpenAI signs 10-gigawatt Ohio data-center lease with Nvidia-backed SB Energy
secondary_headline: Nvidia backstops $105 billion for first phase, takes $1.5 billion
  equity stake
publish_date: '2026-08-17'
lede: OpenAI has signed a 20-year, 10-gigawatt data-center lease in southern Ohio
  with SoftBank's SB Energy, according to The Wall Street Journal. Nvidia will backstop
  the project's first phase and serve as the exclusive chip provider for the first
  half of the campus.
nut_graf: The arrangement is the latest example of chip firms using their balance
  sheets to help artificial-intelligence startups secure enormous computing capacity
  while limiting their own risk exposure.
primary_entities:
- OpenAI
- Nvidia
- SB Energy
- SoftBank
- U.S. Department of Energy
primary_themes:
- AI infrastructure financing
- Data-center development
- Corporate backstop structures
topic_tags:
- artificial intelligence
- computing and information technology
- energy and resource
- economy, business and finance
storyline_nexus: []
storyline_continuity:
  decision: none
  selected_prior_ids: []
  evaluated_prior_ids: []
  authorized_context_prior_ids: []
geographic_location: United States
floor_values_engaged:
- value: accountability_of_power
  intensity: 0.9
- value: equality_fairness
  intensity: 0.6
- value: informed_citizenship
  intensity: 0.3
framework_version: 1.5.0
generation_timestamp: '2026-08-17T06:15:11-07:00'
source_cluster_id: cluster_wsj_2026-08-17_n-lease-for-huge-data-center-in-ohio-wit
gdelt_event_ids: []
consensus_floor_version: v0.3.0
publication_mindspec_version: v0.3.0
license: https://creativecommons.org/publicdomain/zero/1.0/
ai_disclosure: This article was generated algorithmically by Main Street Independent
  from the public sources listed in its Sources section.
ai_generated: true
sources:
  count: 1
  outlets:
  - The Wall Street Journal
  outlet_classes:
  - national_daily
  highest_reliability_tier: 1
  has_originating: true
  has_primary_document: false
figures_aggregate:
  count: 0
  series_ids: []
  sources: []
cross_article_links:
- slug: 2026-08-17-big-tech-holds-3-trillion-in-off-balance-sheet-ai-commitments-filings-show
  relation: related
  confidence: medium
- slug: 2026-08-10-nvidia-reaches-500-billion-ai-financing-deal-with-wall-street-firms
  relation: related
  confidence: medium
- slug: 2026-07-22-openai-raises-cloud-spending-target-to-750-billion-by-2030
  relation: related
  confidence: medium
- slug: 2026-07-30-google-guarantees-15-billion-loan-for-anthropic-s-texas-data-center
  relation: related
  confidence: medium
- slug: 2026-07-04-openai-appears-not-to-have-visited-proposed-uk-stargate-site-records-show
  relation: related
  confidence: medium
- slug: 2026-07-21-utilities-covering-80-of-u-s-power-sign-trump-ai-pledge-enforcement-uncertain
  relation: related
  confidence: medium
- slug: 2026-07-21-openai-names-two-bank-ceos-to-board-ahead-of-ipo
  relation: related
  confidence: medium
- slug: 2026-07-09-news-organizations-ask-court-to-sanction-openai-for-withholding-evidence
  relation: related
  confidence: medium
- slug: 2026-07-07-nscale-lands-900-million-revolving-facility-to-expand-ai-data-centers-in-three-r
  relation: related
  confidence: medium
- slug: 2026-07-07-big-tech-ai-capex-set-to-hit-168-billion-in-q2-as-meta-weighs-renting-out-spare-
  relation: related
  confidence: medium
- slug: 2026-06-08-openai-files-for-ipo-testing-investor-appetite-for-top-ai-lab
  relation: related
  confidence: medium
- slug: 2026-02-27-openai-lands-110-billion-funding-from-amazon-softbank-and-nvidia
  relation: related
  confidence: medium
- slug: 2026-06-11-ai-wealth-boom-drives-san-francisco-home-prices-to-new-highs
  relation: related
  confidence: medium
- slug: 2026-06-08-amazon-and-corning-sign-multibillion-dollar-fiber-optic-deal-for-ai-data-centers
  relation: related
  confidence: medium
- slug: 2026-05-23-openai-trial-ends-without-verdict-but-sheds-light-on-ai-funding-fight
  relation: related
  confidence: medium
- slug: 2026-03-25-openai-foundation-pledges-1b-in-grants-for-health-research-and-ai-impacts
  relation: related
  confidence: medium
- slug: 2026-03-21-doe-unveils-10-gigawatt-ohio-data-center-natural-gas-power-plan
  relation: related
  confidence: medium
- slug: 2026-03-28-microsoft-takes-over-texas-ai-data-center-expansion-after-openai-backs-away
  relation: related
  confidence: medium
- slug: 2026-04-29-amazon-expands-partnership-with-openai-to-develop-ai-agent-platform
  relation: related
  confidence: medium
- slug: 2026-03-24-openai-foundation-pledges-1-billion-in-grants-for-health-research-and-ai-impacts
  relation: related
  confidence: medium
draft: false
related_stories:
- slug: 2026-08-17-big-tech-holds-3-trillion-in-off-balance-sheet-ai-commitments-filings-show
  headline: Big tech holds $3 trillion in off-balance-sheet AI commitments, filings
    show
  publish_date: '2026-08-17'
  relation: related
  strength: 0.6879
- slug: 2026-08-10-nvidia-reaches-500-billion-ai-financing-deal-with-wall-street-firms
  headline: Nvidia reaches $500 billion AI financing deal with Wall Street firms
  publish_date: '2026-08-10'
  relation: related
  strength: 0.6886
- slug: 2026-07-30-google-guarantees-15-billion-loan-for-anthropic-s-texas-data-center
  headline: Google guarantees $15 billion loan for Anthropic's Texas data center
  publish_date: '2026-07-30'
  relation: related
  strength: 0.6566
- slug: 2026-07-22-openai-raises-cloud-spending-target-to-750-billion-by-2030
  headline: OpenAI raises cloud spending target to $750 billion by 2030
  publish_date: '2026-07-22'
  relation: related
  strength: 0.6794
- slug: 2026-07-04-openai-appears-not-to-have-visited-proposed-uk-stargate-site-records-show
  headline: OpenAI appears not to have visited proposed UK Stargate site, records
    show
  publish_date: '2026-07-04'
  relation: related
  strength: 0.6865
- slug: 2026-02-27-openai-lands-110-billion-funding-from-amazon-softbank-and-nvidia
  headline: OpenAI lands $110 billion funding from Amazon, SoftBank and Nvidia
  publish_date: '2026-02-27'
  relation: related
  strength: 0.7371
- slug: 2026-03-28-microsoft-takes-over-texas-ai-data-center-expansion-after-openai-backs-away
  headline: Microsoft takes over Texas AI data center expansion after OpenAI backs
    away
  publish_date: '2026-03-28'
  relation: related
  strength: 0.6589
- slug: 2026-03-25-openai-foundation-pledges-1b-in-grants-for-health-research-and-ai-impacts
  headline: OpenAI Foundation pledges $1B in grants for health research and AI impacts
  publish_date: '2026-03-25'
  relation: related
  strength: 0.6586
---
## OpenAI signs 10-gigawatt Ohio data-center lease with Nvidia-backed SB Energy

**Subtype:** fact

- OpenAI has signed a 20-year, 10-gigawatt data-center lease in southern Ohio with SoftBank's SB Energy, a project expected to become one of the largest artificial-intelligence hubs to date.
- Nvidia agreed to backstop up to $105 billion in value for the first phase of the campus and will serve as the exclusive chip provider for the first half of the site.
- Nvidia will make a $1.5 billion equity investment in SB Energy, the SoftBank majority-owned developer that is working on an IPO targeting $5 billion to $7 billion.
- The southern Ohio campus is partly on a former uranium-enrichment site owned by the U.S. Department of Energy and is expected to generate enough energy to power 7 million homes.
- Commerce Secretary Howard Lutnick and Energy Secretary Chris Wright have been deeply involved in the project.

## Nvidia backstops $105 billion for first phase, takes $1.5 billion equity stake

OpenAI has signed a 20-year, 10-gigawatt data-center lease in southern Ohio with SoftBank's SB Energy — a project partly backed by Nvidia that is expected to become one of the largest artificial-intelligence hubs to date, The Wall Street Journal reported Monday.

The arrangement rests on a financing structure designed to use Nvidia's balance sheet to make the project easier and less expensive to finance. Nvidia agreed to backstop a portion of the value of the completed data center at the site, beginning with the first phase, which represents around 5 gigawatts of power. In return, Nvidia will serve as the exclusive chip provider for the first half of the Ohio campus and take an equity stake in SB Energy, according to people familiar with the matter.

The backstop is structured to give lenders confidence that the data centers would retain value even if OpenAI were no longer the tenant, the people said. If OpenAI were to walk away from the project, SB Energy would first try to lease the site to another customer at the same price; if it could not find a suitable replacement, the firm would try to sell the site, and Nvidia would pay any difference in value, up to $105 billion if the initial phase is completed. The commitment requires a series of mitigation steps before Nvidia would be obligated to make any payments, and it covers completed data centers rather than facilities under construction. Nvidia has the option to expand its commitment later to the full campus.

The deal caps weeks of negotiations. In recent days, OpenAI, SB Energy and Nvidia reworked the scale of the backstop to address investors' concerns about the chip maker's risk exposure, according to people familiar with the matter. Morgan Stanley advised Nvidia.

The structure is the latest example of chip firms using their balance sheets to help AI startups secure the enormous computing capacity their businesses require. Chip firms including Nvidia and Google have increasingly agreed to back arrangements that let startups such as OpenAI and Anthropic sign leases that developers can raise debt against while trying to manage their own risk.

Nvidia initially planned to guarantee all 10 gigawatts of OpenAI's data-center commitments upfront, which could have totaled around $250 billion, the Journal previously reported. Nvidia shares fell 5% in late July after the Journal earlier reported on the discussions of that larger guarantee.

Beyond the backstop, Nvidia is expected to reap commercial benefits from the project, according to people with knowledge of the deal. The company plans to sell hundreds of billions of dollars' worth of chips into the initial phase of the Ohio facility, people with knowledge of the deal said, and has discussed providing financing for OpenAI's purchase of its chips for the campus.

Separately, Nvidia said it agreed to make a $1.5 billion equity investment in SB Energy, the project's developer. SB Energy is majority owned by Japan's SoftBank and counts OpenAI as an investor. The developer is working with bankers on an IPO as soon as next month, aiming to raise between $5 billion and $7 billion, people familiar with that planned deal said.

The southern Ohio campus is partly located on a former uranium-enrichment site owned by the U.S. Department of Energy and is expected to generate enough energy to power 7 million homes, according to the Journal. Commerce Secretary Howard Lutnick and Energy Secretary Chris Wright have been deeply involved in the project.

The data-center campus will rely on the construction of a 9.2-gigawatt natural-gas plant, which is owned by the U.S. government and financed by Japan under a recent trade deal.

## Sources

### src_001 — The Wall Street Journal, national_daily, Tier 1, originating
**Author:** Anissa Gardizy
**Publication date:** 2026-08-17
**Title:** OpenAI Locks In Lease for Huge Data Center in Ohio With Backing From Nvidia
**URL:** https://www.wsj.com/articles/openai-locks-in-lease-for-huge-data-center-in-ohio-with-backing-from-nvidia-7474bb9c
