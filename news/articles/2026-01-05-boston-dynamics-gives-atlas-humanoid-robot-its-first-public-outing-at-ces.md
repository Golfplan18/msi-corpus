---
headline: Boston Dynamics gives Atlas humanoid robot its first public outing at CES
publish_date: '2026-01-05'
lede: Hyundai-owned Boston Dynamics gave its humanoid robot Atlas its first-ever public demonstration Monday at the CES technology showcase in Las Vegas, putting a life-sized, two-armed machine through its paces before a crowd and sharpening a race with Tesla and other rivals to build robots capable of human-like tasks. An engineer remotely piloted the robot during the showcase, though the company said Atlas will operate autonomously in real-world use.
nut_graf: The live debut — paired with a new partnership with Google's DeepMind to supply artificial intelligence to Boston Dynamics robots — signals accelerating commercial ambition in the humanoid robotics sector, even as analysts caution that widespread workplace deployment remains years away.
primary_entities:
- Boston Dynamics
- Hyundai
- Atlas
- Google DeepMind
- Zachary Jackowski
- Alex Panas
- SoftBank
primary_themes:
- humanoid robotics
- artificial intelligence
- manufacturing automation
- CES 2026
- labor and employment
topic_tags:
- robotics
- artificial intelligence
- science and technology
- manufacturing and engineering
- computing and information technology
storyline_nexus:
- ai-industry-regulation
geographic_location: United States
floor_values_engaged:
- value: informed_citizenship
  intensity: 0.6
framework_version: 1.3.0
generation_timestamp: '2026-05-23T00:00:03Z'
source_cluster_id: cluster_ap_2026-01-06_ces-humanoid-robots-atlas-hyundai-boston
gdelt_event_ids: []
consensus_floor_version: v0.3.0
publication_mindspec_version: v0.3.0
license: https://creativecommons.org/publicdomain/zero/1.0/
ai_generated: true
claims:
  count: 9
  hedges:
    contested: 0
    attributed: 3
    reported: 2
    alleged: 0
    appears: 0
    confirmed: 4
  corroboration:
    single_source: 9
    primary_plus_secondary: 0
    primary_document: 0
    one_originating_plus_primary_document: 0
    two_independent: 0
sources:
  count: 1
  outlets:
  - Associated Press
  outlet_classes:
  - wire
  highest_reliability_tier: 1
  has_originating: true
  has_primary_document: false
figures_aggregate:
  count: 0
  series_ids: []
  sources: []
image:
  url: /articles/2026-01-05-boston-dynamics-gives-atlas-humanoid-robot-its-first-public-outing-at-ces.png
  alt: 'Illustration accompanying article: Boston Dynamics gives Atlas humanoid robot its first public outing at CES'
  source: ai_generated
cross_article_links: []
draft: false
---

## Summary
**Subtype:** fact

- The live debut — paired with a new partnership with Google's DeepMind to supply artificial intelligence to Boston Dynamics robots — signals accelerating commercial ambition in the humanoid robotics...
- Hyundai-owned Boston Dynamics gave its humanoid robot Atlas its first-ever public demonstration Monday at the CES technology showcase in Las Vegas,...
- "For the first time ever in public, please welcome Atlas to the stage," said Zachary Jackowski, Boston Dynamics' general manager for humanoid robot...
- An engineer remotely piloted the robot during the showcase. In real-world use, Atlas will move around on its own, Jackowski said.

The live debut — paired with a new partnership with Google's DeepMind to supply artificial intelligence to Boston Dynamics robots — signals accelerating commercial ambition in the humanoid robotics sector, even as analysts caution that widespread workplace deployment remains years away.

Hyundai-owned Boston Dynamics gave its humanoid robot Atlas its first-ever public demonstration Monday at the CES technology showcase in Las Vegas, putting a life-sized, two-armed machine through its paces before a crowd and sharpening a race with Tesla and other rivals to build robots capable of human-like tasks.

"For the first time ever in public, please welcome Atlas to the stage," said Zachary Jackowski, Boston Dynamics' general manager for humanoid robots, as the robot picked itself up from the floor of a hotel ballroom and walked around the stage for several minutes, waving to the crowd and swiveling its head.

An engineer remotely piloted the robot during the showcase. In real-world use, Atlas will move around on its own, Jackowski said.

## Deployment planned for Hyundai's Georgia EV plant

Boston Dynamics said a product version of Atlas is already in production and will be deployed by 2028 at Hyundai's electric vehicle manufacturing facility near Savannah, Georgia, where it will help assemble cars.

Hyundai, the South Korean automaker, holds a controlling stake in Massachusetts-based Boston Dynamics, which it acquired from Japanese tech conglomerate SoftBank in 2021. Google had purchased Boston Dynamics in 2013 before selling it to SoftBank. The company has developed robots for decades and is best known for Spot — a dog-like, four-legged robot and its first commercial product. A group of Spot robots opened Hyundai's CES event by dancing in synchrony to a K-pop song.

## Google's DeepMind returns as an AI partner

Hyundai also announced a new partnership with Google's DeepMind to supply artificial intelligence to Boston Dynamics robots, reviving a connection that predates SoftBank's and Hyundai's successive ownership of the company.

## Analysts urge caution on timeline

Despite the commercial momentum, industry analysts cautioned against overstating how quickly humanoid robots will reshape workplaces.

"I think the question comes back to what are the use cases and where is the applicability of the technology," said Alex Panas, a partner at consultancy McKinsey who helped lead a CES robotics panel that attracted hundreds of people. "In some cases, it may look more humanoid. In some cases, it may not."

Panas said "the software, the chipsets, the communication, all the other pieces of the technology are coming together, and they will create new applications."

Humanoids do not yet have enough dexterity to threaten many human jobs, though a debate over their effects on employment is likely to grow as the machines become more skilled.

## Live demos remain rare in the industry

It is rare for leading robot makers to publicly demonstrate their humanoids. Robotics startups typically prefer to show prototypes in edited videos posted to social media, giving them the opportunity to highlight the machines at their best and omit failures — as when one of Russia's first humanoids fell on its face in November.

Monday's Atlas demonstration appeared flawless. At its conclusion, the prototype swung its arms in a theatrical gesture to introduce a static model of the product version of Atlas, which looked slightly different and was blue in color.

## Plant context: immigration raid in 2025

The Georgia facility where Hyundai plans to test Atlas carries additional context. The same plant was the site of a federal immigration raid in 2025 that led to the arrests of hundreds of workers, including more than 300 South Korean citizens.

## Atomic claims

### c_001
- **Hedge:** confirmed
- **Corroboration:** single_source
- **Subject entities:** Boston Dynamics; Atlas
- **Predicate:** demonstrated_publicly
- **Object:** first public demonstration at CES, Las Vegas (event)
- **Temporal:** 2026-01-05
- **Sources:** src_001

> Boston Dynamics publicly demonstrated its humanoid robot Atlas for the first time at CES on January 5, 2026, in Las Vegas.

### c_002
- **Hedge:** attributed
- **Corroboration:** single_source
- **Subject entities:** Boston Dynamics engineer; Atlas
- **Predicate:** piloted_remotely
- **Object:** demonstration only; autonomous in real-world use (operational_mode)
- **Temporal:** 2026-01-05
- **Sources:** src_001

> An engineer remotely piloted Atlas during the CES demonstration; in real-world use, Atlas will move around on its own.

### c_003
- **Hedge:** attributed
- **Corroboration:** single_source
- **Subject entities:** Atlas; Hyundai
- **Predicate:** will_be_deployed
- **Object:** Hyundai EV facility near Savannah, Georgia, by 2028 (deployment_plan)
- **Temporal:** 2028
- **Sources:** src_001

> A product version of Atlas is already in production and will be deployed by 2028 at Hyundai's electric vehicle manufacturing facility near Savannah, Georgia, to help assemble cars.

### c_004
- **Hedge:** confirmed
- **Corroboration:** single_source
- **Subject entities:** Hyundai; Boston Dynamics; SoftBank
- **Predicate:** acquired
- **Object:** controlling stake in Boston Dynamics from SoftBank (ownership)
- **Temporal:** 2021
- **Sources:** src_001

> Hyundai holds a controlling stake in Massachusetts-based Boston Dynamics, which it acquired from SoftBank in 2021.

### c_005
- **Hedge:** confirmed
- **Corroboration:** single_source
- **Subject entities:** Google; Boston Dynamics; SoftBank
- **Predicate:** acquired_then_sold
- **Object:** Boston Dynamics, sold to SoftBank after 2013 acquisition (corporate_history)
- **Temporal:** 2013
- **Sources:** src_001

> Google bought Boston Dynamics in 2013 before selling it to SoftBank.

### c_006
- **Hedge:** confirmed
- **Corroboration:** single_source
- **Subject entities:** Hyundai; Google DeepMind; Boston Dynamics
- **Predicate:** announced_partnership
- **Object:** AI supply agreement for Boston Dynamics robots (agreement)
- **Temporal:** 2026-01-05
- **Sources:** src_001

> Hyundai announced a new partnership with Google's DeepMind to supply artificial intelligence to Boston Dynamics robots.

### c_007
- **Hedge:** attributed
- **Corroboration:** single_source
- **Subject entities:** Boston Dynamics; Spot
- **Predicate:** developed_and_commercialized
- **Object:** Spot, dog-like robot, first commercial product (product)
- **Temporal:** 2026-01-05
- **Sources:** src_001

> Boston Dynamics has been developing robots for decades and is best known for its first commercial product, the dog-like robot Spot.

### c_008
- **Hedge:** reported
- **Corroboration:** single_source
- **Subject entities:** humanoid robots
- **Predicate:** lack_capability
- **Object:** insufficient dexterity to threaten many human jobs (capability_assessment)
- **Temporal:** 2026-01-05
- **Sources:** src_001

> Humanoids do not yet have enough dexterity to threaten many human jobs.

### c_009
- **Hedge:** reported
- **Corroboration:** single_source
- **Subject entities:** Hyundai EV plant; federal immigration authorities
- **Predicate:** was_site_of
- **Object:** federal immigration raid; arrests of hundreds, including more than 300 South Korean citizens (event)
- **Temporal:** 2025
- **Sources:** src_001

> The Hyundai EV plant near Savannah, Georgia was the site of a federal immigration raid that led to the arrests of hundreds of workers, including more than 300 South Korean citizens.

## Sources

### src_001 — Associated Press, wire, Tier 1, originating
**Author:** Matt O'Brien
**Publication date:** 2026-01-05
**Title:** Hyundai and Boston Dynamics unveil humanoid robot Atlas at CES
**URL:** https://apnews.com/article/ces-humanoid-robots-atlas-hyundai-boston-dynamics-8de7b2470c23f5f22441ad1ad7555136
