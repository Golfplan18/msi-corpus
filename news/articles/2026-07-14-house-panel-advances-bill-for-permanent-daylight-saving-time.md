---
headline: House panel advances bill for permanent daylight saving time
secondary_headline: Sunshine Protection Act cleared for floor vote after rules panel
  approval
publish_date: '2026-07-14'
lede: The House Rules Committee on Monday voted 6-4 to approve a rule advancing the
  Sunshine Protection Act, a bill that would make daylight saving time permanent nationwide.
  The legislation, sponsored by Rep. Vern Buchanan (R-Fla.), would end the twice-yearly
  clock changes and now heads to the full House for a potential vote.
nut_graf: The bill advances after previous efforts to make daylight saving time permanent
  had stalled. The measure has drawn support from state-level legislative action —
  19 states have passed laws to adopt permanent daylight saving time if Congress authorizes
  it — and would require Senate passage and presidential approval to take effect.
primary_entities:
- United States House of Representatives
- Sunshine Protection Act
- Daylight saving time
- Vern Buchanan
- Donald Trump
primary_themes:
- Legislative process
- Time policy
- Bipartisan legislation
topic_tags:
- politics
- government
storyline_nexus:
- trump-climate-energy-rollbacks
geographic_location: United States
floor_values_engaged:
- value: human_life_and_dignity
  intensity: 0.3
- value: truthfulness
  intensity: 0.9
- value: accountability_of_power
  intensity: 0.3
framework_version: 1.4.1
generation_timestamp: '2026-07-14T07:04:30-07:00'
source_cluster_id: cluster_guardian_2026-07-14_us-news-2026-jul-14-house-committee-adva
gdelt_event_ids: []
consensus_floor_version: v0.3.0
publication_mindspec_version: v0.3.0
license: https://creativecommons.org/publicdomain/zero/1.0/
ai_disclosure: This article was generated algorithmically by Main Street Independent
  from the public sources listed in its Sources section.
ai_generated: true
sources:
  count: 1
  outlets:
  - The Guardian
  outlet_classes:
  - national_daily
  highest_reliability_tier: 2
  has_originating: true
  has_primary_document: false
figures_aggregate:
  count: 0
  series_ids: []
  sources: []
cross_article_links:
- slug: 2026-07-10-trump-says-he-will-not-sign-housing-bill-over-stalled-voting-measure
  relation: related
  strength: 0.5853
  confidence: medium
- slug: 2026-07-05-house-subcommittee-advances-ratepayer-protection-act-as-consumer-groups-call-it-
  relation: related
  strength: 0.563
  confidence: medium
- slug: 2026-06-24-trump-abruptly-cancels-signing-ceremony-for-bipartisan-housing-bill
  relation: related
  strength: 0.5823
  confidence: medium
- slug: 2026-03-05-states-pursue-an-end-to-twice-a-year-clock-changes
  relation: related
  strength: 0.798
  confidence: medium
- slug: 2026-05-19-south-carolina-house-approves-gop-leaning-congressional-map-senate-next
  relation: related
  strength: 0.6039
  confidence: medium
- slug: 2026-05-20-south-carolina-house-passes-gop-leaning-congressional-map-after-midnight
  relation: related
  strength: 0.5923
  confidence: medium
- slug: 2026-03-13-what-to-know-about-your-health-as-the-us-springs-forward-sunday
  relation: related
  strength: 0.7001
  confidence: medium
- slug: 2026-04-26-all-nighters-in-congress-create-dysfunction-after-dark
  relation: related
  strength: 0.6319
  confidence: medium
draft: false
image:
  url: /articles/2026-07-14-house-panel-advances-bill-for-permanent-daylight-saving-time.png
  alt: 'Illustration accompanying article: House panel advances bill for permanent
    daylight saving time'
  credit: Main Street Independent (algorithmic)
  source: ai_generated
  license: https://creativecommons.org/publicdomain/zero/1.0/
  disclosure: AI-generated image. Prompt summary and model identifier available in
    metadata.
  ai_model: openrouter:openai/gpt-5.4-image-2
  ai_prompt: hand-drawn, black ink linework, solid white background, fully opaque,
    linework dominant, varied pen technique | square 1:1 composition, full-bleed square
    frame that fills the entire canvas edge to edg
related_stories:
- slug: 2026-03-05-states-pursue-an-end-to-twice-a-year-clock-changes
  headline: States pursue an end to twice-a-year clock changes
  publish_date: '2026-03-05'
  relation: related
  strength: 0.7543
---
## House panel advances bill for permanent daylight saving time

**Subtype:** fact

- The House Rules Committee approved a rule on a 6-4 vote to advance the Sunshine Protection Act, a bill to make daylight saving time permanent nationwide.
- The legislation, sponsored by Rep. Vern Buchanan (R-Fla.), would set daylight saving time as the permanent standard time, eliminating the twice-yearly clock changes.
- The bill has bipartisan support, including backing from President Donald Trump, who called it a "very nice WIN for the Republican Party."
- The House Energy and Commerce Committee previously passed the measure in May by a 48-1 vote.

## Sunshine Protection Act cleared for floor vote after rules panel approval

The House Rules Committee on Monday voted 6-4 to approve a rule teeing up floor consideration of the Sunshine Protection Act, moving the legislation one step closer to a full House vote. The bill would make daylight saving time permanent, ending the practice of changing clocks twice a year.

If enacted, the measure would lead to later sunrises and sunsets year-round, extending daylight in the evening hours during the darker months. The bill advances after previous efforts to make daylight saving time permanent had stalled. According to the National Conference of State Legislatures, 19 states have passed laws that would adopt permanent daylight saving time if Congress authorizes the change.

The legislation has drawn bipartisan support. President Donald Trump wrote on Truth Social after the Energy and Commerce Committee's May vote that the measure would be a "very nice WIN for the Republican Party." Trump said, "We are going with the far more popular alternative, Saving Daylight, which gives you a longer, brighter Day – And who can be against that – This is an easy one!"

The bill still needs approval from the full House and then passage in the Senate before it could become law. Previous attempts to install permanent daylight saving time have stalled at the federal level, though support for ending the clock changes has grown.

Debate continues over whether daylight saving time or standard time should be the fixed standard. Sleep medicine specialists have argued that permanent standard time is more aligned with human circadian rhythms.

Hawaii and most of Arizona do not participate in the current daylight saving time system and would not be affected under the bill. The House Energy and Commerce Committee originally approved the bill in May with a 48-1 vote.

## Sources

### src_001 — The Guardian, national_daily, Tier 2, originating
**Publication date:** 2026-07-14
**Title:** US House committee advances bill to make daylight saving time permanent
**URL:** https://www.theguardian.com/us-news/2026/jul/14/house-committee-advances-daylight-saving-time-bill
