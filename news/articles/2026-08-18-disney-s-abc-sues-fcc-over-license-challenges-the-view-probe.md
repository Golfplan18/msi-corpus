---
headline: Disney's ABC sues FCC over license challenges, 'The View' probe
secondary_headline: License revocations are rare; last tied to programming was in
  1969
publish_date: '2026-08-18'
lede: Disney's ABC has sued the Federal Communications Commission, alleging the agency's
  efforts to challenge its broadcast licenses and regulate the talk show 'The View'
  are illegal and an effort to quash speech the Trump administration finds objectionable.
  The lawsuit, filed in federal court in Washington, D.C., asks for a temporary restraining
  order and preliminary injunction to halt the FCC's license renewal process.
nut_graf: The suit escalates a dispute that began last year when FCC Chairman Brendan
  Carr opened a probe into Disney's diversity, equity and inclusion initiatives. ABC
  says the agency's review of its eight local stations is retaliation for on-air criticism
  of the Trump administration, while Carr says the scrutiny concerns the company's
  DEI practices, not its programming.
primary_entities:
- The Walt Disney Company
- ABC
- Federal Communications Commission
- Brendan Carr
- The View
- Jimmy Kimmel Live
primary_themes:
- Broadcast license regulation
- Government scrutiny of media speech
- Diversity, equity and inclusion initiatives
topic_tags:
- arts, culture, entertainment and media
- civil rights
- law
- law enforcement
- political process
storyline_nexus: []
storyline_continuity:
  decision: none
  selected_prior_ids: []
  evaluated_prior_ids: []
  authorized_context_prior_ids: []
geographic_location: United States
floor_values_engaged:
- value: accountability_of_power
  intensity: 0.9
- value: equality_fairness
  intensity: 0.9
- value: truthfulness
  intensity: 0.6
framework_version: 1.5.0
generation_timestamp: '2026-08-18T06:51:59-07:00'
source_cluster_id: cluster_wsj_2026-08-18_disneys-abc-sues-fcc-over-challenge-to-i
gdelt_event_ids: []
consensus_floor_version: v0.3.0
publication_mindspec_version: v0.3.0
license: https://creativecommons.org/publicdomain/zero/1.0/
ai_disclosure: This article was generated algorithmically by Main Street Independent
  from the public sources listed in its Sources section.
ai_generated: true
sources:
  count: 1
  outlets:
  - The Wall Street Journal
  outlet_classes:
  - national_daily
  highest_reliability_tier: 1
  has_originating: true
  has_primary_document: false
figures_aggregate:
  count: 0
  series_ids: []
  sources: []
cross_article_links:
- slug: 2026-06-22-abc-launches-on-air-campaign-against-fcc-license-review-the-view-probe
  relation: related
  confidence: medium
- slug: 2026-06-25-fcc-chair-says-abc-running-misinformation-campaign-on-view-probe
  relation: related
  confidence: medium
- slug: 2026-07-31-fcc-chair-defends-abc-actions-says-broadcasters-owe-public-service
  relation: related
  confidence: medium
- slug: 2026-05-29-abc-owned-stations-call-early-fcc-license-review-unlawful-and-unconstitutional
  relation: related
  confidence: medium
- slug: 2026-05-10-abc-says-fcc-is-trying-to-chill-free-speech-in-dispute-over-the-view
  relation: related
  confidence: medium
- slug: 2026-05-10-abc-accuses-trump-fcc-of-attempting-to-chill-free-speech-in-dispute-over-the-vie
  relation: related
  confidence: medium
- slug: 2026-05-27-fcc-s-sole-democrat-warns-media-companies-against-yielding-to-trump-pressure-on-
  relation: related
  confidence: medium
- slug: 2026-05-26-fcc-s-sole-democrat-warns-media-companies-against-yielding-to-trump
  relation: related
  confidence: medium
- slug: 2026-06-29-dish-dbs-prepares-chapter-11-bankruptcy-filing-as-early-as-tuesday
  relation: related
  confidence: medium
- slug: 2026-06-15-florida-sues-tiktok-over-state-child-social-media-access-law
  relation: related
  confidence: medium
- slug: 2026-06-03-trump-turns-federal-agencies-into-arm-of-presidency-lobbying-jumps-70
  relation: related
  confidence: medium
- slug: 2026-05-19-judges-appear-split-in-dispute-between-ai-company-anthropic-and-pentagon
  relation: related
  confidence: medium
- slug: 2026-05-17-onion-bids-to-take-over-infowars-as-jones-fights-liquidation-in-court
  relation: related
  confidence: medium
- slug: 2026-04-22-supreme-court-questions-fcc-process-in-verizon-and-at-t-location-data-case
  relation: related
  confidence: medium
- slug: 2026-03-20-fcc-approves-nexstar-tegna-merger-as-states-directv-sue-to-block-deal
  relation: related
  confidence: medium
- slug: 2026-03-14-newsguard-sues-ftc-over-investigation-into-news-rating-service
  relation: related
  confidence: medium
- slug: 2026-03-24-students-sue-university-of-alabama-over-suspension-of-campus-magazines
  relation: related
  confidence: medium
- slug: 2026-03-18-trump-team-pressures-media-to-cover-iran-war-the-administration-s-way
  relation: related
  confidence: medium
- slug: 2026-04-03-trump-administration-appeals-ruling-blocking-pentagon-action-vs-anthropic
  relation: related
  confidence: medium
- slug: 2026-02-24-states-sue-trump-administration-over-vaccine-rollback
  relation: related
  confidence: medium
draft: false
related_stories:
- slug: 2026-06-22-abc-launches-on-air-campaign-against-fcc-license-review-the-view-probe
  headline: ABC launches on-air campaign against FCC license review, 'The View' probe
  publish_date: '2026-06-22'
  relation: related
  strength: 0.8778
- slug: 2026-06-25-fcc-chair-says-abc-running-misinformation-campaign-on-view-probe
  headline: FCC chair says ABC running 'misinformation campaign' on 'View' probe
  publish_date: '2026-06-25'
  relation: related
  strength: 0.8454
- slug: 2026-07-31-fcc-chair-defends-abc-actions-says-broadcasters-owe-public-service
  headline: FCC chair defends ABC actions, says broadcasters owe public service
  publish_date: '2026-07-31'
  relation: related
  strength: 0.6834
- slug: 2026-05-29-abc-owned-stations-call-early-fcc-license-review-unlawful-and-unconstitutional
  headline: ABC-owned stations call early FCC license review 'unlawful and unconstitutional'
  publish_date: '2026-05-29'
  relation: related
  strength: 0.8325
- slug: 2026-05-10-abc-accuses-trump-fcc-of-attempting-to-chill-free-speech-in-dispute-over-the-vie
  headline: ABC accuses Trump FCC of attempting to chill free speech in dispute over
    'The View'
  publish_date: '2026-05-10'
  relation: related
  strength: 0.8469
- slug: 2026-05-10-abc-says-fcc-is-trying-to-chill-free-speech-in-dispute-over-the-view
  headline: ABC says FCC is trying to chill free speech in dispute over ‘The View’
  publish_date: '2026-05-10'
  relation: related
  strength: 0.835
- slug: 2026-05-27-fcc-s-sole-democrat-warns-media-companies-against-yielding-to-trump-pressure-on-
  headline: FCC's sole Democrat warns media companies against yielding to Trump pressure
    on speech
  publish_date: '2026-05-27'
  relation: related
  strength: 0.7428
- slug: 2026-05-26-fcc-s-sole-democrat-warns-media-companies-against-yielding-to-trump
  headline: FCC’s sole Democrat warns media companies against yielding to Trump
  publish_date: '2026-05-26'
  relation: related
  strength: 0.718
---
## Disney's ABC sues FCC over license challenges, 'The View' probe

**Subtype:** fact

- Disney's ABC has sued the FCC, alleging the agency's broadcast-license review and "The View" probe are illegal retaliation for speech the Trump administration finds objectionable.
- The suit, filed in federal court in Washington, D.C., asks a court to halt the FCC's license renewal process and requests a hearing from the D.C. Circuit Court.
- FCC Chairman Brendan Carr has said the license review concerns Disney's diversity, equity and inclusion initiatives, not the network's programming.
- ABC's eight local stations face an early license renewal review the FCC launched in April, shortly after President Trump called for "Jimmy Kimmel Live" host Jimmy Kimmel to be fired.

## License revocations are rare; last tied to programming was in 1969

Disney's ABC has sued the Federal Communications Commission, alleging the agency's efforts to challenge its broadcast licenses and regulate its talk show "The View" are illegal and amount to an effort to quash speech the Trump administration finds objectionable. In a suit filed in federal court in Washington, D.C., the network is asking for a temporary restraining order and preliminary injunction to halt the FCC's license renewal process, and it has requested a hearing from the D.C. Circuit Court.

In the suit, ABC said: "Again and again, the Administration has attacked ABC's speech—the stories its journalists report and the viewpoints its network programs air. Over time, those attacks have escalated into express demands that ABC be stripped of its broadcast licenses because of its speech."

Disney said the administration "has waged a retaliatory campaign against ABC for a single reason: it disapproves of what ABC broadcasts." In a separate line from the filing, Disney said it had no other option than to sue, facing "no alternative means to eliminate these ongoing and immediate threats other than total capitulation to the Administration's demands." Walt Disney shares fell 3.14%.

The dispute began last year, when FCC Chairman Brendan Carr launched a probe into Disney's diversity, equity and inclusion initiatives to determine whether they violated the agency's prohibition on unlawful discrimination. Executives inside ABC told The Wall Street Journal they believed the DEI probe was really a response to commentary on some of the network's shows, including the late-night program "Jimmy Kimmel Live" and the daytime show "The View," both of which have been harsh critics of the Trump administration.

Those suspicions escalated in April when, shortly after Kimmel made disparaging remarks about Trump and the president called for the host to be fired, the FCC said it was launching an early license renewal review for ABC's eight local broadcast stations.

Carr has said the license renewal review isn't about speech but rather about the company's DEI initiatives. ABC has said Carr's FCC is trying to control who the network can have on its shows; Carr countered that the agency isn't trying to control who appears on shows but is "enforcing the provisions" passed by Congress.

ABC argued in the suit that if the FCC were to go forward with a license renewal hearing inside the agency, "an adverse outcome is all but guaranteed."

The FCC this year also launched a separate inquiry into whether "The View" qualifies for an exemption from equal-time rules because it is considered a bona fide news program. The rules apply to radio and broadcast television and require non-news shows that have political candidates on during elections to also bring on their opponents. "The View" previously received an exemption from the FCC. Such a finding could make it more challenging for a broadcast network show to have candidates on its programs, since it would have to provide equal time to all opponents no matter how viable their campaigns are.

Broadcast license reviews and revocations are rare. The last time the FCC revoked a broadcast license related to a station's programming was in 1969, when a Jackson, Mississippi station lost its license for defending segregation on the air. In 1980, the FCC revoked the broadcast licenses of media company RKO for business misconduct and lack of candor.

The FCC and Carr didn't immediately respond to a request for comment.

## Sources

### src_001 — The Wall Street Journal, national_daily, Tier 1, originating
**Author:** Joe Flint
**Publication date:** 2026-08-18
**Title:** Disney’s ABC Sues FCC Over Challenge to Its Broadcast Licenses
**URL:** https://www.wsj.com/business/media/disneys-abc-sues-fcc-over-challenge-to-its-broadcast-licenses-64e8f794
